...
def test_get_desktop_images(monkeypatch, test_client):
    ...
    assert len(data) == 3
    assert data == MOCK_STOCK_CARDS + MOCK_USER_CARDS
...
def test_get_desktop_images_user_kind(monkeypatch, test_client):
    ...
    assert response.status_code == 200
    assert response.json() == MOCK_USER_CARDS
    assert captured["user_id"] == "local-default-user-alice"


def test_get_desktop_images_invalid_kind_returns_400(test_client):
