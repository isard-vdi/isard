...
# The ``user_networks`` router is currently disabled (commented out in
# ``api/__init__.py``), so ``/item/user/networks*`` is not registered and
# every assertion below would 404. The tests are kept as a record of the
# wire contract for when the router is re-enabled.
#
