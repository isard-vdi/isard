...
class TestDomainsOnUpdate:
    ...
    @pytest.fixture
    def handler(self):
        from handlers.domains import DesktopDomainHandler
...
class TestDomainsDelegate:
    ...
    @pytest.mark.asyncio
    @patch("handlers.domains.Helpers._is_frontend_desktop_status", return_value=True)
    ...
    @pytest.mark.asyncio
    @patch("handlers.domains.Helpers._is_frontend_desktop_status", return_value=False)
...
@pytest.mark.asyncio
@patch("handlers.domains.DesktopsProcessed._parse_desktop", side_effect=lambda d: d)
...
@pytest.mark.asyncio
@patch("handlers.domains.Logging")
@patch("handlers.domains.Scheduler")
@patch("handlers.domains.DesktopsProcessed._parse_desktop", side_effect=lambda d: d)
...
class TestDomainsNoneRoomRegression:
    ...
    async def test_template_domain_emit_skips_when_room_is_none(self):
        from handlers.domains import TemplateDomainHandler
