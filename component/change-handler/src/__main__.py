...
# Tables handled by the default BaseHandler (admins-only passthrough).
# Any table not listed here must have a dedicated handler subclass.
PASSTHROUGH_TABLES = ("scheduler_jobs", "user_storage", "users_migrations")
...
assert set(PASSTHROUGH_TABLES) <= set(
    handler_map
), "PASSTHROUGH_TABLES must all appear in handler_map"
for _t in PASSTHROUGH_TABLES:
    assert (
        isinstance(handler_map[_t], BaseHandler)
        and type(handler_map[_t]) is BaseHandler
    ), f"Passthrough table {_t!r} should be handled by BaseHandler, not a subclass"
    ...
    while True:
        ...
        try:
            ...
            async for message in pubsub.listen():
                ...
                table = message["channel"]
                subscriber = TABLE_TO_SUBSCRIBER.get(table)
                if subscriber is None:
                    logger.warning(f"No subscriber for table: {table}")
                    continue

                try:
                    envelope = subscriber.parse(message["data"])
                except Exception as e:
                    logger.error(f"Failed to parse Redis message: {e}")
                    continue

                logger.info(f"[{table}] Received change: {envelope.change}")
                ...
                try:
                    await handler.handle(envelope.change)
                except Exception as e:
