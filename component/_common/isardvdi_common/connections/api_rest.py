...
class ApiRest:
    def __init__(self, service="isard-api", base_url=None):
        if base_url:
            ...
            if service in ("isard-api", "isard-apiv4"):
                actual_server = os.environ.get("API_DOMAIN", "isard-apiv4")
    ...
    def wait_for(self, max_retries=-1, timeout=1):
        while max_retries:
            try:
                ...
                # Timeouts, SSL, invalid responses, etc. — log the full
                # traceback and keep retrying while retries remain.
