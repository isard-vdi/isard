"""Unit tests for the pure interval-algebra helpers of
...
into one (``join_consecutive_plans``). They touch no DB. Times are plain ints.
"""
...
def _diff(segs, expected):
    return ReservablesPlannerCompute.intersect_different_subitem_plan(
        segs, expected_subitems=expected
    )
...
def test_same_profile_two_cards_overlap_sums_units():
    ...
    out = ReservablesPlannerCompute.intersect_same_subitem_plan(
...
def test_adjacent_windows_merge_into_one_available():
    ...
    out = ReservablesPlannerCompute.join_consecutive_plans(
        [_seg(10, 13, 1, "a"), _seg(13, 16, 1, "a")]
    )
...
def test_zero_unit_windows_keep_event_type():
    ...
    out = ReservablesPlannerCompute.join_consecutive_plans([seg])
    assert len(out) == 1
