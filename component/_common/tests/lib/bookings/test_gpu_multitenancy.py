...
from isardvdi_common.lib.bookings import gpu_multitenancy
from isardvdi_common.lib.bookings.gpu_multitenancy import _card_visible

_visible = _card_visible


def _filter(plans, payload, visible):
    # Inject the only DB dependency (visible_gpu_card_ids) as a stub.
    with mock.patch.object(
        gpu_multitenancy, "visible_gpu_card_ids", lambda _payload: visible
    ):
        return gpu_multitenancy.filter_gpu_plans_by_category(plans, payload)
