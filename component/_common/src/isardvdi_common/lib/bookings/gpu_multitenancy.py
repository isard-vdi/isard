...
class _Rdb(RethinkSharedConnection):
    """Connection anchor: the helpers below are module-level functions (the
    consumers pass plain dicts), so they borrow the shared pool through this
    minimal subclass instead of an instance class of their own."""
...
def gpu_owner_category(card, profile=None):
    ...
    with _Rdb._rdb_context():
        row = r.table("gpus").get(card).run(_Rdb._rdb_connection)
...
def category_uses_global_pool(category_id):
    ...
    if not category_id:
        return True
    with _Rdb._rdb_context():
        category = r.table("categories").get(category_id).run(_Rdb._rdb_connection)
    if not category:
        return True
...
def visible_gpu_card_ids(payload):
    ...
    with _Rdb._rdb_context():
        cards = list(r.table("gpus").pluck("id", "category").run(_Rdb._rdb_connection))
    ids = set()
    for card in cards:
...
def filter_reservables_by_category(reservables, payload, keep_ids=None):
    ...
    with _Rdb._rdb_context():
        cards = list(
            r.table("gpus").pluck("id", "profiles_enabled").run(_Rdb._rdb_connection)
        )
    visible_enabled = set()
    for card in cards:
