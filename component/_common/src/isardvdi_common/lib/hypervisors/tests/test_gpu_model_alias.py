...
!4496). The twin module pulls in the whole isardvdi_common package on
import, so — mirroring the upstream test — we evaluate ONLY the
self-contained ``_MODEL_ALIASES`` + ``_model_alias`` block extracted from
the module source via runpy. The hypervisor twin in
docker/hypervisor/src/lib/gpu_discovery.py must stay consistent; the
parity test below enforces that the two alias maps are byte-for-byte
equal.
"""
...
_API_SRC = os.path.join(_HERE, "..", "hypervisors.py")
...
def _load_resync_fn():
    ...
    so it can be unit-tested without importing the whole twin module."""
    src = open(_API_SRC).read()
    tree = ast.parse(src)
    for node in ast.walk(tree):
        if (
    ...
    raise AssertionError("gpu_card_metadata_resync not found in hypervisors.py")
