...
from isardvdi_common.helpers.error_base import RequestObj
...
class Helpers(RethinkSharedConnection):
    ...
    @classmethod
    def parse_dav_propstat(cls, xml_data):
    ...
    @classmethod
    def start_login_auth(cls, provider_id):
        global login_thread
        if login_thread:
            login_thread.kill()
        with cls._rdb_context():
            provider = r.table("user_storage").get(provider_id).run(cls._rdb_connection)
    ...
    @classmethod
    def get_login_auth_callback(cls, token, login_url, provider_id):
...
class NextcloudApi:
    ...
    def remove_user(self, user_id):
        if user_id == self.user:
            log.debug(f"Nextcloud {self.user} admin user cannot be removed")
            return
        url = self.apiurl + "users/" + user_id + "?format=json"
    ...
    def enable_user(self, user_id):
        if user_id == self.user:
            log.debug(f"Nextcloud {self.user} admin user cannot be enabled")
            return
    ...
    def disable_user(self, user_id):
        if user_id == self.user:
            log.debug(f"Nextcloud {self.user} admin user cannot be disabled")
            return
    ...
    def add_group(self, group_id, skip_if_exists=True):
        ...
        if result["ocs"]["meta"]["statuscode"] == 102:
            if not skip_if_exists:
                log.error("Group " + group_id + " already exists")
                raise Error(
                    "conflict",
