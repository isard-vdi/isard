...
try:
    ...
    sessions_client = app.sessions_client
except (ImportError, AttributeError):
    # No Flask `api.app` available (non-Flask service, or Flask app not
    # yet initialized) — fall back to a freshly created gRPC client.
    from isardvdi_common.connections.grpc_client import create_sessions_client

    sessions_client = create_sessions_client("isard-sessions", 1312)
...
def get_user_session_id(user_id):
    try:
        return sessions_client.GetUserSession(
...
def revoke_user_session(user_id):
    try:
        ...
        return sessions_client.Revoke(sessions_pb2.RevokeRequest(id=session_id))
