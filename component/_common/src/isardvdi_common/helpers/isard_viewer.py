...
class IsardViewer(RethinkSharedConnection):
    ...
    @classmethod
    def viewer_jwt(cls, category_id, desktop_id, minutes=240, admin_role=False):
        return jwt.encode(
            {
    ...
    def get_rdp_file(self, ip, username, password):
        fixed = ViewersConfig.rdp_file_viewer()["fixed"] % (ip, username, password)
        custom = ViewersConfig.rdp_file_viewer()["custom"]
        consola = fixed + "\n" + custom
    ...
    def get_rdp_gw_file(
        ...
        fixed = ViewersConfig.rdpgw_file_viewer()["fixed"] % (
            ip,
            proxy_video,
            proxy_port,
            jwt_token,
        ...
        custom = ViewersConfig.rdpgw_file_viewer()["custom"]
    ...
    def get_spice_file(self, domain, port, vmPort):
        ...
        custom = ViewersConfig.spice_file_viewer()["custom"]
