...
class Quotas(RethinkCustomBase):
    ...
    def get_applied_quota(cls, user_id):
        user = Caches.get_document(
            "users", user_id, ["id", "category", "group", "quota"]
        )
        group = Caches.get_document("groups", user["group"], ["name", "quota"])
        category = Caches.get_document(
            "categories", user["category"], ["name", "quota"]
        )
        ...
        elif group.get("quota"):
            return {
                "quota": group["quota"],
                "restriction_applied": "group_quota",
            }
        elif category.get("quota"):
            return {
    ...
    def Get(cls, user_id, started_info=True):
        ...
        used = {
            ...
            "deployment_users": user_deployment_users,
            "started_deployment_desktops": cls.get_started_deployment_desktops(user_id),
        }

        if started_info:
            started_desktops = cls.get_started_desktops(
    ...
    @classmethod
    def desktop_create(cls, user_id, quantity=1):
        ...
        cls.check_field_quotas_and_limits(
            user,
            "desktops",
            quantity,
            quota_error,
    ...
    @classmethod
    def volatile_create(cls, user_id, quantity=1):
        user = Caches.get_cached_user_with_names(user_id)
    ...
    @classmethod
    def template_create(cls, user_id, quantity=1):
        user = Caches.get_cached_user_with_names(user_id)
        with cls._rdb_context():
            group_quantity = (
                r.table("domains")
    ...
    @classmethod
    def media_create(cls, user_id, media_size=False, quantity=1):
        user = Caches.get_cached_user_with_names(user_id)
    ...
    @classmethod
    def check_field_quotas(cls, user, quota_key, quantity, quota_error):
        # We get the user applied quota and currently used info to check the quota
        user_quota_data = copy.deepcopy(cls.Get(user_id=user["id"], started_info=False))
        if user_quota_data["quota"]:
            ...
            cls.check_quota(user_quota_data, quota_key, quota_error)
    ...
    def check_field_limits(
        ...
        group = Caches.get_document(
            "groups", user["group"], ["name", "quota", "limits"]
        )
    ...
    def check_field_quotas_and_limits(
        ...
        if check_quota:
            cls.check_field_quotas(user, quota_key, quantity, quota_error)

        if check_limits:
            cls.check_field_limits(
                user,
                quota_key,
                quantity,
                group_quantity,
    ...
    @classmethod
    def check_quota(cls, user, quota_key, quota_error):
    ...
    @classmethod
    def check_limits(cls, item, quota_key, quantity, limits_error):
    ...
    def get_started_desktops(cls, query_id, query_index, owner_only=False):
        # Status that are considered in the running quota
        started_status = [
            "Started",
            "Starting",
    ...
    def deployment_create(
        ...
        user = Caches.get_cached_user_with_names(owner_id)
        ...
        # Check the amount of users in the deployment
        if users:
            quota_error = {
                "error_description": user["name"]
                + " quota exceeded for users in deployment",
                "error_description_code": "deployment_users_new_user_quota_exceeded",
            }
            cls.check_field_quotas(
                user,
                "deployment_users",
            ...
            for group_id, users in groups_users.items():
                group = Caches.get_document(
                    "groups", group_id, ["name", "quota", "limits"]
                )
                if group.get("limits"):
                    with cls._rdb_context():
                        group_domains = (
            ...
            for category_id, users in categories_users.items():
                ...
                if category.get("limits"):
                    ...
                    if category_domains + len(users) > category["limits"].get(
                        "desktops"
                    ):
                        raise Error(
                            "precondition_required",
    ...
    @classmethod
    def deployment_update(cls, users, desktops_len, owner_id):
        user = Caches.get_cached_user_with_names(owner_id)

        # TODO: shoud it be possible to add or remove desktops from an existing deployment?
    ...
    @classmethod
    def get_hardware_allowed(cls, payload, domain_id=None):
        return cls.user_hardware_allowed(payload, kind=None, domain_id=domain_id)

    @classmethod
    def get_hardware_kind_allowed(cls, payload, kind):
    ...
    def get_shutdown_timeouts(cls, payload, desktop_id=None):
        return QuotasProcess.get_shutdown_timeouts(payload, desktop_id=None)
    ...
    def user_hardware_allowed(cls, payload, kind=None, domain_id=None):
        ...
        if not kind or kind == "interfaces":
            dict["interfaces"] = Alloweds.get_items_allowed(
                payload,
                "interfaces",
                query_pluck=["id", "name", "description"],
                order="name",
        ...
        if not kind or kind == "graphics":
            dict["graphics"] = Alloweds.get_items_allowed(
                payload,
                "graphics",
                query_pluck=["id", "name", "description"],
                order="name",
        ...
        if not kind or kind == "videos":
            dict["videos"] = Alloweds.get_items_allowed(
                payload,
                "videos",
                query_pluck=["id", "name", "description"],
                order="name",
        ...
        if not kind or kind == "boot_order":
            dict["boot_order"] = Alloweds.get_items_allowed(
                payload,
                "boots",
                query_pluck=["id", "name", "description"],
                order="name",
        ...
        if not kind or kind == "qos_id":
            dict["qos_id"] = Alloweds.get_items_allowed(
                payload,
                "qos_disk",
                query_pluck=["id", "name", "description"],
                order="name",
        ...
        if not kind or kind in ["isos", "floppies"]:
        ...
        if not kind or kind == "quota":
            quota = cls.get_applied_quota(payload["user_id"])
        else:
            quota = {}
    ...
    @classmethod
    def check_users_group_quota(cls, user_ids: list, group_id):
        with cls._rdb_context():
            group = (
                r.table("groups")
                .get(group_id)
        ...
        for user_id in user_ids:
            user_quota_data = cls.Get(user_id=user_id, started_info=False)
        ...
        gqp = QuotasProcess.process_group_limits(group_id)
        if gqp == False:
            return
        group_used = {
