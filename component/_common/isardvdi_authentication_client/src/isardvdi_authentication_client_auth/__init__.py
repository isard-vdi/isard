"""Hand-written helpers for the generated isard-authentication client.
...
``isardvdi_apiv4_client_auth`` — the isard-authentication service
validates the same ``kid=isardvdi`` admin-role HS256 tokens that apiv4
does (``API_ISARDVDI_SECRET``).

TODO: Consolidate JWT/error helpers into a shared
``isardvdi_service_client_auth`` package once a third service client
joins the generation pipeline.
"""
...
from isardvdi_authentication_client import AuthenticatedClient
...
AuthApiError = _ApiError
raise_for_status = _raise_for_status

__all__ = ["AuthApiError", "build_client", "raise_for_status"]
...
def build_client(
    ...
    *,
    ...
    """Return an ``AuthenticatedClient`` ready to call isard-authentication.
