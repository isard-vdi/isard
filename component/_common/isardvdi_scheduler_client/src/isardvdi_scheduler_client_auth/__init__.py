...
the same ``kid=isardvdi`` admin-role HS256 tokens that apiv4,
authentication and notifier do (``API_ISARDVDI_SECRET``).

TODO: Consolidate JWT/error helpers into a shared
``isardvdi_service_client_auth`` package now that a fourth service
...
if TYPE_CHECKING:
    from isardvdi_scheduler_client import AuthenticatedClient

# Re-export shared error helpers under the local namespace so call
# sites don't need to import from the apiv4 package.
SchedulerApiError = _ApiError
raise_for_status = _raise_for_status

__all__ = ["SchedulerApiError", "build_client", "raise_for_status"]
...
def build_client(
    ...
    *,
    ...
    """Return an ``AuthenticatedClient`` ready to call isard-scheduler.
