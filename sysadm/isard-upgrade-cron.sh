...
then
	echo "Error: DOCKER_IMAGE_TAG differs from remote tracked branch" >&2
	exit 1
fi
...
git stash pop
./build.sh
services="$(docker-compose -f docker-compose$CONFIG_NAME.yml config --services | sed '/^isard-hypervisor$/d')"
