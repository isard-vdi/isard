...
if not os.environ.get("TESTING_WEBAPP") == "true":
    ...
    if not os.path.exists("./install/.wizard"):
        exit(1)
    ...
    def send_build(path):
        return send_from_directory(
            os.path.join(app.root_path, "bower_components/gentelella/build"), path
        )

    @app.route("/vendors/<path:path>")
    def send_vendors(path):
        return send_from_directory(
            os.path.join(app.root_path, "bower_components/gentelella/vendors"), path
        )
    ...
    def send_font_linux(path):
        return send_from_directory(
            os.path.join(app.root_path, "bower_components/font-linux/assets"), path
        )
