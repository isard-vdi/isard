...
class isard:
        ...
        try:
            if data["name"] == "status":
                ...
                if data["value"] == "Starting":
                    if (
                        float(
                            app.isardapi.get_user_quotas(current_user.username)["rqp"]
                        )
                        >= 100
                    ):
                        return (
                            json.dumps(
                                {
                                    ...
                                    + " "
                                    + data["pk"]
                                    + " can't be started because you have exceeded quota",
                                    "icon": "warning",
                                    "type": "warning",
                                }
                            ),
                ...
                json.dumps(
                    {
                        ...
                        "text": item + " " + data["pk"] + " can't be started now",
                        "icon": "warning",
                        "type": "error",
                    }
                ),
    ...
    def update_domain_network(self, user, id, interface_id=False, remote_addr=False):
        try:
            if interface_id:
                ...
                allowed_ifaces = self.get_alloweds(
                    user, "interfaces", pluck=["id", "net"]
                )
    ...
    def update_table_value(self, table, id, field, value):
        with app.app_context():
            return self.check(
                r.table(table).get(id).update({field: value}).run(db.conn), "replaced"
            )
    ...
    def show_disposable(self, client_ip):
        ...
        if disposables_config["active"]:
            ...
            for d in disposables:
                for net in d["nets"]:
                    if IPAddress(client_ip) in IPNetwork(net):
                        return d
        return False
    ...
    def get_group_domains(self, group, filterdict=False):
        if not filterdict:
            filterdict = {"kind": "desktop"}
        with app.app_context():
            domains = self.f.table_values_bstrap(
                r.table("domains")
                .get_all(group, index="group")
                .filter(filterdict)
                .without("xml")
                .run(db.conn)
            )
    ...
    def get_domain(self, id, human_size=False, flatten=True):
        ...
        with app.app_context():
            domain = (
                r.table("domains")
                .get(id)
                .without("viewer", "xml", "history_domain", "progress")
                .run(db.conn)
            )
        try:
            if flatten:
                ...
                try:
                    idchain.append(
                        list(
                            r.table("domains")
                            .filter(
                                lambda disks: disks["hardware"]["disks"][0]["file"] == f
                            )
                            .pluck("id", "name")
                            .run(db.conn)
                        )[0]
    ...
    def get_template_kind(self, user, d):
        if (
            d["allowed"]["roles"] is False
            and d["allowed"]["categories"] is False
            and d["allowed"]["groups"] is False
        ):
            if (d["allowed"]["users"] and user in d["allowed"]["users"]) or d[
                "allowed"
            ]["users"] is False:
                return "Private"
        return "Shared"
            ...
            while backing != "":
                ...
                try:
                    backing = list(
                        r.table("domains")
                        .filter(
                            lambda disks: disks["hardware"]["disks"][0]["file"]
                            == backing
                        )
                        .run(db.conn)
                    )[0]["backing_chain"]
    ...
    def template_kind_toggle(self, user, id):
        with app.app_context():
            ...
            if (
                not allowed["roles"]
                and not allowed["categories"]
                and not allowed["groups"]
            ):
                if (allowed["users"] and user in allowed["users"]) or allowed[
                    "users"
                ] is False:
                    ...
                    return self.check(
                        r.table("domains")
                        .get(id)
                        .update({"allowed": allowed, "kind": kind})
                        .run(db.conn),
                        "replaced",
                    )
            allowed = {
            ...
            if v is not False and len(v):
                with app.app_context():
                    allowed[k] = list(
                        ...
                        .pluck("id", "name")
                        .map(
                            lambda doc: doc.merge({"text": doc["name"]}).without("name")
                        )
                        .run(db.conn)
                    )
        return allowed
        ...
        with app.app_context():
            ...
            if filter_type == "base":
                ...
                data = (
                    ...
                    .filter(
                        lambda d: d["allowed"]["roles"] != False
                        or d["allowed"]["groups"] != False
                        or d["allowed"]["categories"] != False
                    )
                    .order_by("name")
                    .group("category")
                    .pluck({"id", "name", "allowed"})
    ...
    def get_all_table_allowed_term(
        self, table, kind, field, value, user, pluck="default"
    ):
        if pluck is "default":
    ...
    def user_relative_media_path(self, user, filename):
        with app.app_context():
            userObj = (
                r.table("users").get(user).pluck("id", "category", "group").run(db.conn)
            )
    ...
    def new_tmpl_from_domain(self, from_id, part_dict, user):
        ...
        if r.table("domains").get(part_dict["id"]).run(db.conn) is not None:
            return False

        part_dict["create_dict"]["hardware"]["disks"] = tmpl_dict["create_dict"][
            "hardware"
        ]["disks"]
        ...
        tmpl_dict["create_dict"]["hardware"]["disks"][0] = {
            "file": dir_disk + "/" + disk_filename,
            "parent": "",
        }  # , 'bus':part_dict['create_dict']['hardware']['diskbus']}
        ...
        with app.app_context():
            return self.check(
                ...
                .update(
                    {
                        "create_dict": tmpl_dict["create_dict"],
    ...
    def new_domain_disposable_from_tmpl(self, client_ip, template):
        ...
        with app.app_context():
            ...
            exists_domain = list(
                r.table("domains")
                .filter(
                    lambda domain: domain["id"].match(
                        "^_disposable_" + parsed_name + "_"
                    )
                )
                .run(db.conn)
            )
            if len(exists_domain) > 1:
                log.error(
                    "More than one disposable domain match beginning with _disposable_"
                    + parsed_name
                )
                return False
            if len(exists_domain) == 1:
                ...
                r.table("domains").get(exists_domain[0]["id"]).update(
                    {"status": "StoppingAndDeleting"}
                ).run(db.conn)
            # Create new disposable
            userObj = (
                r.table("users")
                .get("disposable")
                .pluck("id", "category", "group")
                .run(db.conn)
            )
        ...
        new_rnd = old_rnd
        while new_rnd == old_rnd:
            new_rnd = "".join([random.choice(chars) for i in range(4)])

        dir_disk, disk_filename = self.get_disk_path(
            userObj, parsed_name + "_" + new_rnd
        )
        create_dict["hardware"]["disks"] = [
            {"file": dir_disk + "/" + disk_filename, "parent": parent_disk}
        ]
        ...
        new_domain = {
            "id": "_disposable_" + parsed_name + "_" + new_rnd,
            "name": parsed_name,
            ...
            "server": dom["server"],
            "os": dom["os"],
            "create_dict": {"hardware": create_dict["hardware"], "origin": template},
            "hypervisors_pools": create_dict["hypervisors_pools"],
            "allowed": {
                "roles": False,
                "categories": False,
