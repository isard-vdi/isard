...
template = r.db('isard').table('domains').get(template_id).pluck('id','name','kind','user').run()
derivated = list(r.db('isard').table('domains').pluck('id','name','kind','user','status','parents').filter(lambda derivates: derivates['parents'].contains(template_id)).run())
doms=[]
for d in derivated:
...
def template_delete_list(template_id):
    template = r.db('isard').table('domains').get(template_id).pluck('id','name','kind','user','status','parents').run()
    derivated = list(r.db('isard').table('domains').pluck('id','name','kind','user','status','parents').filter(lambda derivates: derivates['parents'].contains(template_id)).run())
    fancyd = []
    for d in derivated:
        fancyd.append({'title': d['id'],
                        'expanded': True,
