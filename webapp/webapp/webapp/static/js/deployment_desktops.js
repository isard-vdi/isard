...
$(document).ready(function() {
    ...
    $('#mactions').on('change', function () {
        ...
        if(table.rows('.active').data().length){
            ...
            var text = text +"You are about to "+action+" these desktops:\n\n "+names
        }else{ 
            $.each(table.rows({filter: 'applied'}).data(),function(key, value){
                ids.push(value['id']);
                deployments.push(value['tag'])
            });
            var text = text + "You are about to "+action+" "+table.rows({filter: 'applied'}).data().length+" desktops!\n All the desktops in list!"
        }
            ...
            if (form.parsley().isValid()){
                ...
                if (template !=''){
                    ...
                    if('tag_visible' in data){
                        console.log('tag visible in data')
                        data['tag_visible']=true
                    }else{
                        console.log('tag visible not in data')
