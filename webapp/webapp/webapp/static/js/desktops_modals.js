...
function filterTemplate(obj) {
	...
	$.ajax({
		type: "GET",
		url:"/isard-admin/desktops/filterTemplate/" + kind,
...
function templateFiltersFill(kind){
		...
		$.ajax({
			type: "GET",
			url:"/isard-admin/desktops/getDistinc/"+"user"+"/"+kind,
		...
		$.ajax({
			type: "GET",
			url:"/isard-admin/desktops/getDistinc/"+"category"+"/"+kind,
			success: function(dictOptions)
			{
			  var options= Object.keys(dictOptions);
			  var content='<option value="-1">Select:</option>';
		...
		$.ajax({
			type: "GET",
			url:"/isard-admin/desktops/getDistinc/"+"group"+"/"+kind,
			success: function(dictOptions)
			{
			  var options= Object.keys(dictOptions);
			  var content='<option value="-1">Select:</option>';
...
function filterTemplateBy(field,element) {
	...
	$.ajax({
		type: "GET",
		url:"/isard-admin/desktops/filterTemplate/public_template/"+field+"/" + value,
		success: function(templates)
		{
		  var categories= Object.keys(templates);
		  var total=0;
