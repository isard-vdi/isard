...
$(document).ready(function() {
    ...
    table['builders']=$('#builders_tbl').DataTable({
			"ajax": {
				"url": "/isard-admin/admin/updates/builders/",
				"dataSrc": ""
			},
			"language": {
				"loadingRecords": '<i class="fa fa-spinner fa-pulse fa-3x fa-fw"></i><span class="sr-only">Loading...</span>',
    ...
    $('#builders_tbl').find(' tbody').on( 'click', 'button', function () {
        ...
        switch($(this).attr('id')){
            case 'btn-download':
                api.ajax('/isard-admin/admin/updates/download/builders/'+datarow['id'],'POST',{}).done(function(data) {
                      table['builders'].ajax.reload();
                  }); 
                break;
            case 'btn-delete':
                api.ajax('/isard-admin/admin/updates/delete/builders/'+datarow['id'],'POST',{}).done(function(data) {
                   table['builders'].ajax.reload();
    ...
    table['virt_builder']=$('#virt_builder_tbl').DataTable({
			"ajax": {
				"url": "/isard-admin/admin/updates/virt_builder/",
				"dataSrc": ""
			},
			"language": {
				"loadingRecords": '<i class="fa fa-spinner fa-pulse fa-3x fa-fw"></i><span class="sr-only">Loading...</span>',
    ...
    $('#virt_builder_tbl').find(' tbody').on( 'click', 'button', function () {
        ...
        switch($(this).attr('id')){
            case 'btn-download':
                api.ajax('/isard-admin/admin/updates/download/virt_builder/'+datarow['id'],'POST',{}).done(function(data) {
                      table['virt_builder'].ajax.reload();
                  }); 
                break;
            case 'btn-delete':
                api.ajax('/isard-admin/admin/updates/delete/virt_builder/'+datarow['id'],'POST',{}).done(function(data) {
                   table['virt_builder'].ajax.reload();
                   //~ table['virt_install'].row('#'+datarow['id']).remove().draw();
                  }); 
                break;
