...
function getUsageGrouping(conf, startDate, endDate) {
    $.ajax({
        url: '/api/v4/admin/usage/grouping/' + conf.grouping,
        type: 'GET',
        dataType: 'json',
        contentType: 'application/json'
...
function getUsageCredits(conf, grouping, startDate, endDate) {
    $.ajax({
        type: "GET",
        url: `/api/v4/admin/usage/credits/category/${grouping.item_type}/${$('meta[id=user_data]').attr('data-categoryId')}/${conf.grouping}/${moment(startDate).utc().format('YYYY-MM-DD')}/${moment(endDate).utc().format('YYYY-MM-DD')}`,
        dataType: 'json',
        contentType: "application/json",
