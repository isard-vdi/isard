            ...
            try:
                with app.app_context():
                    for c in r.table('domains').without('xml','history_domain').changes(include_initial=False).run(db.conn):
                        ...
                        # if event=='desktop_data' and 'tag_visible' in data.keys() and data['tag_visible']==False:
                        #         ## Should delete to user if it's connected and only show to advanced user owner
                        #         ## We are unable to take the viewer from user, the only way is to stop all desktops as this
                        #         ## will generate a new viewer next time they are started.
                        #         event='desktop_delete'

                        original_event=event
                        if 'tag_visible' in data.keys() and data['tag_visible']==False:
                            if c['old_val'] == None or ('tag_visible' in c['old_val'] and c['old_val']['tag_visible']==True):
                                event='desktop_delete'
                                socketio.emit(event, 
                                                json.dumps(data), 
