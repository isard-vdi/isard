...
from .functions import weighted_choice, backing_chain_cmd
from .functions import exec_remote_list_of_cmds,exec_remote_cmd,size_format,get_threads_names_running
...
def create_cmds_disk_template_from_domain(path_template_disk,path_domain_disk,user_owner='qemu',group_owner='qemu',clustersize='4k'):
    ...
    cmds1.append({'title':'verify_touch'          ,'cmd':'stat -c \'mountpoint:%m group:%G user:%U rights:%A\' {}'.format(touch_test_path)})
    ...
    cmds3 = list()
    cmds3.append({'title':'create_disk_domain_from_new_template' ,'cmd':create_disk_from_base_cmd(path_domain_disk,path_template_disk)})
...
def verify_output_cmds3(cmds_done,path_domain_disk,path_template_disk,id_domain):
    ...
    if len(d['err']) > 0 and error != 'Hard':
        log.error('Backing chain query fail in new disk domain {}. Something was wrong. Error: {}'.format(path_domain_disk,d['err']))
        log.error('cmd: {}, out: {}, err: {}'.format(d['cmd'],d['out'],d['err']))
        error = 'Crashed'
    elif error is None:
    ...
    if len(d['err']) > 0 and error != 'Hard':
        log.error('Backing chain query fail in disk template {}. Something was wrong. Error: {}'.format(path_template_disk,d['err']))
        log.error('cmd: {}, out: {}, err: {}'.format(d['cmd'],d['out'],d['err']))
        error = 'Crashed'
    elif error is None:
...
def verify_output_cmds2(cmds_done,path_domain_disk,path_template_disk,id_domain):
    ...
    if len(d['err']) > 0:
        log.error('template disk {} doesn\'t exist??. Something was wrong. Error: {}'.format(path_template_disk,d['err']))
    ...
    if len(d['err']) > 0:
        ...
        if (new_template_disk_created == True):
            ...
            log.error('Where is Domain disk {}? The disk would have moved to {} but it is nowhere!!'.format(path_domain_disk,path_template_disk))
            error = 'Crashed'
    else:
        #domain disk remain in place
        file_size_domain_disk = int(d['out'])
        if (new_template_disk_created == True):
            log.error('Domain disk {} would have to be moved, but it is in place. Template disk {} have beeen created, but with all the data??'.format(path_domain_disk,path_template_disk))
            log.error('Domain disk {} Size: {} ({})'.format(path_domain_disk,file_size_domain_disk,size_format(file_size_domain_disk)))
            log.error('Template disk {} Size: {} ({})'.format(path_template_disk,file_size_template_disk,size_format(file_size_template_disk)))
            error = 'Crashed'
        else:
            log.error('Domain disk {} would have to be moved, but it is in place. Template disk {} not created, but domain disk with all the data??'.format(path_domain_disk,path_template_disk))
            log.error('Domain disk {} Size: {} ({})'.format(path_domain_disk,file_size_domain_disk,size_format(file_size_domain_disk)))
            error = 'Hard'
...
def verify_output_cmds1_template_from_domain(cmds_done,path_domain_disk,path_template_disk,id_domain):
    ...
    if len(d['err']) > 0:
        log.error('domain disk {} doesn\'t exist or permissions access error. Domain:{}. Error: {}'.format(path_domain_disk,id_domain,d['err']))
        log.debug('cmd: {}, out: {}, err: {}'.format(d['cmd'],d['out'],d['err']))
        error_severity = 'Hard'
    else:
    ...
    if len(d['err']) > 0 and error_severity != 'Hard':
        log.error('directory for template disk {} can not be created or permissions access error. Domain:{}. Error: {}'.format(path_template_disk,id_domain,d['err']))
        log.debug('cmd: {}, out: {}, err: {}'.format(d['cmd'],d['out'],d['err']))
        error_severity = 'Hard'
    ...
    if len(d['err']) > 0 and error_severity != 'Hard':
        ...
        log.debug('disk {} size: {} , template filesystem df: {}'.format(path_domain_disk,size_format(disk_size_bytes),size_format(df_bytes)))
    ...
    if error_severity == None and move_tool != None:
        ...
        if move_tool == 'rsync':
            ...
            cmd_to_move = 'rsync -aP --inplace --bwlimit=30000 --remove-source-files "{}" "{}"'.format(path_domain_disk,path_template_disk)
...
def backing_chain(path_disk,disk_operations_hostname,json_format=True):
    ...
    if len(d['err']) == 0:
        ...
        log.error('backing_chain info for disk {} fail when executing in host {} and command is {}'.format(path_disk,VDESKTOP_DISK_OPERATINOS,cmd))
