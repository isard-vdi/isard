    ...
    def update_status_hyps_rethink(self):
        ...
        if self.try_connect_hyp() is False:
            ...
            if self.hyp_obj.connected:
                ...
                dict_last_hyp_status = self.fifo_recent_hyp_stats[-1] if len(self.fifo_recent_hyp_stats) > 0 else None

                if dict_last_hyp_status is not None:
                    time_elapased_from_last_hyp_stats = dict_hyp_status['when'] - dict_last_hyp_status['when']
                    ...
                    if time_elapased_from_last_hyp_stats < self.rate_allowed_diff_between_samples * self.polling_interval:
                        ...
                        log.error(
                                'time elapsed is greater than polling interval {} * {} from previous hypervisor stats in  {}'.
                                    format(self.polling_interval, self.rate_allowed_diff_between_samples,
                                           self.hostname))
                        self.recent_domains_stats = {}
                        dict_hyp_status['cpu_percent'] = {'user': 0.0, 'iowait': 0.0, 'kernel': 0.0, 'used': 0.0,
                                                          'idle': 0.0}
                ...
                # values in %, from 0 to 100
                selected_hyp_values['cpu_used'] = dict_hyp_status['cpu_percent']['used']
                selected_hyp_values['cpu_iowait'] = dict_hyp_status['cpu_percent']['iowait']
                ...
                for name, status in self.hyp_obj.domain_stats.items():
                    if name in dict_hyp_status['domains']:
                        ...
                        dict_domain_last = self.recent_domains_stats[name][-1] \
                            if len(self.recent_domains_stats) > 0 else None
                        ...
                        self.recent_domains_stats[name].append(dict_domain)
                        insert_db_domain_status(dict_domain)

        self.fifo_recent_hyp_stats.append(dict_hyp_status)
