...
# Stand in a parent package carrying the names hypervisors.py imports from it,
# so the real module loads without engine.services.db.db, which connects to
# RethinkDB at import time (hence the conftest stubbing the package wholesale).
# hypervisors.py also imports rethinkdb.errors, which a MagicMock cannot supply.
_saved = {
    _m: _sys.modules.get(_m)
    for _m in ("engine.services.db", "engine.services.db.hypervisors", "rethinkdb")
}
...
_spec = _ilu.spec_from_file_location(
    ...
    str(ENGINE_SRC / "services" / "db" / "hypervisors.py"),
)
