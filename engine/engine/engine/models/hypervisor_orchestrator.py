...
from engine.services.db import new_rethink_connection,close_rethink_connection, delete_table_item
...
def add_hyper_to_db(hyp_id,
    ...
    if cap_disk:
        for hp in hypervisor['hypervisors_pools']:
            ...
            result = r.table('hypervisors_pools').get(hp).update({'paths':paths,'enabled':False}).run(r_conn)
...
class HypervisorsOrchestratorThread(threading.Thread):
    def __init__(self, name, t_workers, queues_object, t_events,
                 t_long_operations, q_long_operations,
                 t_disk_operations, q_disk_operations,
                 polling_interval=10):
        ...
        while self.stop is False:
            ...
            try:
                ...
                if action['type'] == 'thread_hyp_worker_dead':
                    ...
                    move_actions_to_others_hypers(hyp_id,self.d_queues,
                ...
                if action['type'] == 'thread_disk_operations_dead':
                    ...
                    if self.q_disk_operations[hyp_id].empty() is False:
                        ...
                        move_actions_to_others_hypers(hyp_id,d,
                                                      remove_stopping=True,
                                                      remove_if_no_more_hyps=True)
    ...
    def activate_disk_long_operations(self,hyp_id,timeout=10):
        ...
        if launch_disk_operations is True:
            ...
            self.q_disk_operations[hyp_id] = launch_disk_operations_thread(hyp_id, d['hostname'], d['user'], d['port'], self.q_actions)
        ...
        if launch_long_operations is True:
            ...
            self.q_long_operations[hyp_id] = launch_long_operations_thread(hyp_id, d['hostname'], d['user'], d['port'], self.q_actions)

        return True

    def activate_hyp(self,hyp_id,timeout=10):
        t_worker, q_worker = launch_thread_worker(hyp_id, self.t_events.q_event_register, self.q_actions)
...
class HypervisorChangesThread(threading.Thread):
    ...
    def run(self):
        ...
        for c in r.table('hypervisors').pluck('id',
                                              'capabilities',
                                              'enabled',
                                              'status',
                                              'thread_status',
                                              'hypervisors_pools'
                                              ).merge({'table': 'hypervisors'}).changes(). \
                union(r.table('engine').pluck('status_all_threads').merge({'table': 'engine'}).changes()) \
                .run(self.r_conn):
            ...
            # hypervisor created
                ...
                if c['new_val'].get('table', False) == 'hypervisors':
                    ...
                    logs.main.debug(pprint.pformat(c))
                    if c['new_val']['status'] == 'Offline' and c['new_val']['enabled'] == True:
                        action = {}
                        ...
                        action['hypervisors_pools'] = c['new_val'].get('hypervisors_pools')
                        self.q_orchestrator.put(action)
            else:
                if c['new_val'].get('table', False) == 'hypervisors':
                    logs.main.debug('hypervisor fields modified in rethink')
                    logs.main.debug(pprint.pformat(c))
                    if c['old_val']['enabled'] == False and c['new_val']['enabled'] == True:
                        action = {}
                        action['type'] = 'enable_hyper'
                        action['hyp_id'] = c['new_val'].get('id')
                        action['capabilities'] = c['new_val'].get('capabilities')
                        ...
                        action['hypervisors_pools'] = c['new_val'].get('hypervisors_pools')
                        self.q_orchestrator.put(action)
                    if c['old_val']['enabled'] == True and c['new_val']['enabled'] == False:
                        action = {}
                        ...
                        action['hypervisors_pools'] = c['new_val'].get('hypervisors_pools')
                        self.q_orchestrator.put(action)
                    if c['old_val']['enabled'] == False and c['new_val']['status'] == 'Deleting' and \
                            c['old_val']['status'] in ['Error','Offline']:
