...
class DomainXML(object):
        ...
        if xml_tree.xpath('/domain/devices/interface'):
            ...
            for tree in xml_tree.xpath('/domain/devices/interface'):
                ...
                if tree.xpath('bandwidth'):
                    ...
                    bandwidth_xml = indent(etree.tostring(bandwith_tree, encoding='unicode'))
    ...
    def add_interface(self, type_interface, mac, id_domain, id_interface, model_type='virtio', net='default', qos=False):
        ...
        if type_interface == 'bridge':
            ...
            log.error('type_interface of interface incorrect when adding interface in xml')
            return -1
        ...
        if type(qos) is dict:
            ...
            bandwidth_xml = indent(xmltodict.unparse(bandwidth_dict, full_document=False))
...
def update_xml_from_dict_domain(id_domain, xml=None):
    ...
    update_table_field('domains',id_domain,'hardware_from_xml',hw_updated,merge_dict=False)
...
def populate_dict_hardware_from_create_dict(id_domain):
    ...
    if len(list_interfaces_mac) < len(list_interfaces_id):
        ...
        create_dict['hardware']['interfaces_mac'] = list_interfaces_mac[:len(list_interfaces_id)]
        update_domain_dict_create_dict(id_domain,create_dict)
