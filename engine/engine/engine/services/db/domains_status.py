...
def insert_db_domain_status(dict_domain_status):
    ...
    rtable.insert(dict_domain_status).run(r_conn, durability="soft", noreply=True)
    close_rethink_connection(r_conn)
...
def get_all_domain_status(name, start=None, stop=None, history=False):
    ...
    if start and stop:
        results = (
            rtable.filter(obj)
            .filter(lambda s: start <= s["when"] and s["when"] <= stop)
            .order_by(r.desc("when"))
            .run(r_conn)
        )
    elif start:
