        ...
        try:
            if self.m.t_background != None:
                ...
                if self.m.t_background.is_alive():
                    ...
                    d_engine[
                        'event_thread_is_alive'] = self.m.t_events.is_alive() if self.m.t_events != None else False
                    d_engine['broom_thread_is_alive'] = self.m.t_broom.is_alive() if self.m.t_broom != None else False
                    d_engine['download_changes_thread_is_alive'] = self.m.t_downloads_changes.is_alive() \
                        if self.m.t_downloads_changes != None else False

                    d_engine['changes_hyps_thread_is_alive'] = self.m.t_changes_hyps.is_alive() \
                        if self.m.t_changes_hyps != None else False
                    d_engine[
                        'changes_domains_thread_is_alive'] = self.m.t_changes_domains.is_alive() if self.m.t_changes_domains != None else False
