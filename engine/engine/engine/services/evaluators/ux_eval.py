...
class UXEval(EvaluatorInterface):
    def __init__(self, user_id, id_pool, dd, templates, hyps, params):
        ...
        self.ux_scorers = {
            "execution_time": {"min": 0, "max": 30, "weight": 60, "unit": "percent"},
            ...
            "cpu_usage_mean": {"min": 0, "max": 30, "weight": 20, "unit": "percent"},
        }
        self.total_weights_ux_scorers = sum(
            v["weight"] for v in self.ux_scorers.values()
        )
    ...
    def _calcule_ux(self):
        ...
        if self.calcule_ux:
            for t in self.templates:
                ...
                if len(domains) >= len(
                    self.hyps
                ):  # Enought domains for run in parallel?
                    self._initial_ux_parallel(t["id"], domains)
                else:
                    self._initial_ux_sequencial(t["id"], domains[0]["id"])

                if self.save_ux:
                    initial_ux = {"id": self.code, "initial_ux": self.initial_ux}
                    insert_eval_initial_ux(initial_ux)
                logs.eval.debug(
                    "INITIAL_UX: {}".format(pformat(self.initial_ux[t["id"]]))
                )
            sleep(35)
        else:
    ...
    def _initial_ux_parallel(self, template_id, domains):
        ...
        for i in range(len(self.hyps)):
            ...
            hyp = self.hyps[i]
            logs.eval.info(
                "Calculing ux for template: {} in hypervisor {}. Domain: {}".format(
                    template_id, hyp.id, domain_id
                )
            )
            update_domain_forced_hyp(domain_id, hyp.id)
            threads[i] = Thread(
                target=self._get_stats_background, args=(domain_id, results, i, hyp)
            )
            threads[i].start()
        for i in range(len(self.hyps)):
            threads[i].join()
    ...
    def _start_domains(self):
        ...
        while start_domains <= total_domains:
            ...
            while i < start_domains:
                ...
                threads[i] = Thread(
                    target=self._calcule_ux_background,
                    args=(domain_id, results, i, step),
                )
                threads[i].start()
                i += 1
    ...
    def _calcule_ux_background(self, domain_id, results, thread_id, step):
        ...
        try:
            ...
            logs.eval.info(
                "_calcule_ux_background: domain:{}, thread_id: {}, hyp_id: {}".format(
                    domain_id, thread_id, hyp_id
                )
            )
            # Wait until stop
            ...
            stats, et, stop_by_timeout = self._wait_stop(
                domain_id, hyp, timeout=timeout
            )
            ...
            self.sender.send(
                hyp_id + ".inc_hyp_cpu_usage", increment["cpu_hyp_usage"]["mean"]
            )
            self.sender.send(
                hyp_id + ".inc_hyp_cpu_iowait", increment["cpu_hyp_iowait"]["mean"]
            )
            # Calcule some data
            data = self._calcule_data_from_ux(
                domain_id, hyp_id, template_id, ux, increment
            )
    ...
    def _analyze_step_results(self, results_timeouts):
        results = [
            r[0] for r in results_timeouts if r
        ]  # Remove None's, produced by errors
        timeouts = [
            r[1] for r in results_timeouts if r and r[1] is True
        ]  # Get any True value
        a = np.array(results)
        ...
        for t in self.templates:
            ...
            for hyp in self.hyps:
                c = np.where(
                    (a[:, 1] == hyp.id) & (a[:, 2] == t["id"])
                )  # Query rows by hyp_id and template_id
                tmp = a[c]
                if len(tmp) > 0:
                    hyp_stats = list(
                        np.round(np.mean(tmp[:, 3:].astype(np.float), axis=0), 2)
                    )
                    statistics_names = [
                        "inc_cpu_hyp_iowait_stdev_percent",
                    ...
                    data_results[t["id"]][hyp.id] = {
                        sn: hyp_stats[self.data_ux_names.index(sn)]
                        for sn in statistics_names
                    }
                    data_results[t["id"]][hyp.id]["inc_execution_time_percent"] = round(
                        (hyp_stats[1] - 1) * 100, 2
                    )
                    data_results[t["id"]][hyp.id]["score"] = hyp_stats[-1]
                    data_results[t["id"]][hyp.id]["n_domains"] = len(tmp)
                    data_results[t["id"]][hyp.id]["vcpu_cpu_rate"] = round(
                        2 * len(tmp) / hyp.info["cpu_threads"], 2
                    )
    ...
    def _wait_stop(self, domain_id, hyp, timeout=150):
        ...
        logs.eval.debug(
            "Execution_time: {}, stop_by_time_out: {}, i_value: {}, timeout: {}".format(
                execution_time, stop_by_timeout, i, timeout
            )
        )
    ...
    def _get_inc(self, ux, initial_ux):
        ...
        for name in self.names:
            ...
            for stat in self.inc_statistics:
                initial = (
                    0.01 if initial_ux[name][stat] == 0 else initial_ux[name][stat]
                )
