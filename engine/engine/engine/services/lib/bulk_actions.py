...
def create_domain_from_template(
    name_normalized, id_template, id_user, description="", only_dict=False
):

    regex = re.compile("^[a-zA-Z0-9-_.]*$")
    if regex.match(name_normalized) is None:
        logs.bulk.error(
            "name {} for domain is not normalized, only lower chars, letters and _ are allowed".format(
                name_normalized
            )
        )
        return False
    ...
    if dict_template["kind"] == "desktop":
        logs.bulk.error(
            "domain {} is a desktop, can not derivate from a desktop only from template or base".format(
                id_template
            )
        )
        return False

    if get_domain(id_new_domain) != None:
        logs.bulk.error(
            "domain {} can not be created because it already exists in database".format(
                id_new_domain
            )
        )
        return False
    ...
    if dict_user is None:
        logs.bulk.error(
            "user {} does not exist in database, can not create domains ".format(
                id_user
            )
        )
        return False
    ...
    path_relative_disk = "{}/{}/{}/{}.qcow2".format(
        u["category"], u["group"], id_user, name_normalized
    )
    ...
    d_new["create_dict"]["hardware"]["disks"][0]["parent"] = t["hardware"]["disks"][0][
        "file"
    ]
    ...
    if only_dict is True:
        ...
        if result["inserted"] == 1:
            logs.bulk.info(
                "domain {} created from bulk operation".format(id_new_domain)
            )
            return True
        else:
            logs.bulk.error(
                "error inserting domain {} in database".format(id_new_domain)
            )
            return False
