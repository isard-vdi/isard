import json
from engine.services.db import insert_table_dict,update_table_field, delete_table_item
json_new_hyper = """
...
new_media_dict = new_media = {'accessed': 1630392808.6059213,
...
new_domain_from_iso = {'allowed': {'categories': False,
                                   'groups': False,
                                   'roles': False,
                                   'users': False},
                       'category': 'default',
                       'create_dict': {'create_from_virt_install_xml': 'win10Virtio',
                                       'forced_hyp': False,
                                       'hardware': {'boot_order': ['iso'],
                                                    'diskbus': 'virtio',
                                                    'disks': [{'file': 'default/default/local/admin-admin/srcd01.qcow2',
                                                               'size': '1G'}],
                                                    'floppies': [],
                                                    'graphics': ['default'],
                                                    'interfaces': ['default'],
                                                    'isos': [{
                                                                 'id': '_local-default-admin-admin-systemrescue-8.04-amd64.iso'}],
                                                    'memory': 1572864,
                                                    'vcpus': 1,
                                                    'videos': ['default']},
...
import subprocess
import os
def create_rsync_cmd(src_path,dst_path,verbose=True,show_progress=True,bwlimit=False):
    cmd = 'rsync -a'
    if verbose:
        cmd += 'v'
    if len(bwlimit) > 0:
...
def mv_rsync_with_ssh(src_path,
                      dst_path,
                      hostname='isard-hypervisor',
                      user='root',
                      port=22,
                      bwlimit=''):
    cmd = create_rsync_cmd(src_path,
                           dst_path,
                           bwlimit=bwlimit)
    ssh_template = """ssh -oBatchMode=yes -p {port} {user}@{hostname} """ \
                   f""" "ls -lh {src_path}; rm -f /isard/random_10MB_copy; """ + cmd + '"'

    ssh_command = ssh_template.format(port=port,
                                      user=user,
                                      hostname=hostname,
                                      src_path=src_path,
                                      dst_path=dst_path)
