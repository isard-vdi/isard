...
class Populate(object):
    def __init__(self):
        ...
        self.cfg = cfg.cfg()
        try:
            self.conn = r.connect(self.cfg['RETHINKDB_HOST'], self.cfg['RETHINKDB_PORT'],
                                  self.cfg['RETHINKDB_DB']).repl()
    ...
    def config(self):
        ...
        if not r.table_list().contains('config').run(self.conn):
            ...
            self.result(r.table('config').insert([{'id': 1,
                                                   ...
                                                   'grafana': {'active': False, 'url': '', 'hostname': 'isard-grafana',
                                                               'carbon_port': 2004, "interval": 5},
                                                   'version': 0,
    ...
    def users(self):
        if not r.table_list().contains('users').run(self.conn):
            ...
            if r.table('users').get('admin').run(self.conn) is None:
                ...
                self.result(r.table('users').insert(usr, conflict='update').run(self.conn))
        ...
        if not r.table_list().contains('groups').run(self.conn):
            ...
            if r.table('groups').get('default').run(self.conn) is None:
                self.result(r.table('groups').insert([{'id': 'default-default',
                                                       ...
                                                       'enrollment': {'manager': False, 'advanced': False,
                                                                      'user': False},
                                                       'name': 'Default',
    ...
    def hypervisors_status(self):
        ...
        if not r.table_list().contains('hypervisors_status_history').run(self.conn):
            ...
            r.table_create('hypervisors_status_history', primary_key="id").run(self.conn)
    ...
    def virt_install(self):
        if not r.table_list().contains('virt_install').run(self.conn):
            ...
            self.result(r.table('virt_install').insert(self.update_virtinstalls()).run(self.conn))
        return True
