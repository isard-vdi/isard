...
func SignUserMigrationRequiredToken(secret string, userID string) (string, error) {
	tkn := jwt.NewWithClaims(signingMethod, &UserMigrationRequiredClaims{
		TypeClaims: TypeClaims{
			RegisteredClaims: &jwt.RegisteredClaims{
				// TODO: This should be maybe configurable
				ExpiresAt: jwt.NewNumericDate(time.Now().Add(60 * time.Minute)),
			...
			KeyID: keyID,
			Type:  TypeUserMigrationRequired,
		},
		UserID: userID,
	})
	...
	ss, err := tkn.SignedString([]byte(secret))
	if err != nil {
		return "", fmt.Errorf("sign the user migration required token: %w", err)
	}
