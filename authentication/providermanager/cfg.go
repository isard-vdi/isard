...
func (c *cfgWatcher) watchGlobal(ctx context.Context, wg *sync.WaitGroup, sess r.QueryExecutor) {
	log := c.log.With().Str("config", "global").Logger()

	watcher := db.NewWatcher(&log, c.reloadInterval, 1024, func(ctx context.Context) (model.Config, error) {
		cfg := model.Config{}
	...
	wg.Go(func() {
		for {
			...
			case change := <-watcher.Changes():
				c.handleGlobalChange(change)
			}
		}
	})
}

func (c *cfgWatcher) handleGlobalChange(change db.Change[model.Config]) {
	cfg := change.Old
	newCfg := change.New
...
func (c *cfgWatcher) watchCategories(ctx context.Context, wg *sync.WaitGroup, sess r.QueryExecutor) {
	log := c.log.With().Str("config", "categories").Logger()

	watcher := db.NewWatcher(&log, c.reloadInterval, 1024, func(ctx context.Context) (map[string]model.CategoryConfigEntry, error) {
		return model.CategoryConfigurationsLoad(ctx, sess)
	})
	...
	initialCfg := <-watcher.Changes()
	c.log.Debug().Strs("categories", slices.Sorted(maps.Keys(watcher.Current()))).Msg("loaded initial categories authentication configuration")

	c.handleCategoriesChange(initialCfg)
	...
	wg.Go(func() {
		for {
			...
			case change := <-watcher.Changes():
				c.handleCategoriesChange(change)
			}
		}
	})
}

func (c *cfgWatcher) handleCategoriesChange(change db.Change[map[string]model.CategoryConfigEntry]) {
	cfgCategories := change.Old
	newCfgCategories := change.New
