...
``api_reservables`` cannot be imported bare (api/__init__ boots Flask and a
RethinkDB connection), so — mirroring ``api_hypervisors_gpu_model_test.py`` — we
extract ONLY the self-contained ``_pci_id_to_sysfs`` and ``_tag_vgpus_with_groups``
...
_SRC = os.path.join(os.path.dirname(__file__), "api_reservables.py")
