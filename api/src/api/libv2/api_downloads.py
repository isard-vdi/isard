...
class Downloads(object):
    ...
    def updateFromConfig(self):
        ...
        self.url=cfg['url']
        self.code=cfg['code']
        self.private_code=False if 'private_code' not in cfg.keys() else cfg['private_code']

    def is_conected(self):
        try:
            req= requests.get(self.url,allow_redirects=False, verify=False, timeout=10)
    ...
    def get_user_path(self,user):
        return user['category']+'/'+user['group']+'/'+user['provider']+'/'+user['uid']+'-'+user['username']+'/'
    ...
    def download_desktop(self,desktop_id,user_id):
        with app.app_context():
            if r.table('domains').get(desktop_id).run(db.conn) is not None: return False
        desktop=self.formatDomains([self.getNewKindId('domains',user_id,desktop_id)],user_id)[0]
        with app.app_context():
