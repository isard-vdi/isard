...
class ApiDesktopsPersistent():
    ...
    def NewFromScratch(self,name,user_id,
                        description='Created from api v3',
                        disk_user=False,
                        disk_path=False,disk_path_selected="/isard/groups",disk_bus="virtio",disk_size=False,
                        disks=False, 
                        isos=[], # ['_local-default-admin-admin-systemrescue-8.04-amd64.iso']
                        boot_order=["disk"],
                        vcpus=2,memory=4096,
                        graphics=["default"],videos=["default"],interfaces=["default"],
                        opsystem="windows",
                        icon="circle-o",
                        image="",
                        forced_hyp=False,
        ...
        if user == None: raise UserNotFound

        json_user = {'user':user['id'],
        ...
        if xml is None:
            ...
            if vi == None: raise VirtInstallNotFound
            domain['create_dict']['create_from_virt_install_xml'] = virt_install_id
            json_xml = {'create_dict':{'create_from_virt_install_xml':virt_install_id},
                        'xml':vi['xml']}
        else:
            json_xml = {'xml':xml}
        ...
        if disks:
            try:
                for disk in disks:
                    if not disk.get('file'): raise DisksFileError
                    if not disk.get('path_selected'): raise DisksPathselectedError
            except:
        ...
        json_disks={"create_dict":{"hardware":{"disks":disks}},
        ...
        for iso in isos:
            ...
            if dbiso == None: raise MediaNotFound
        ...
        for graphic in graphics:
            ...
            if dbgraphic == None: raise GraphicNotFound
        ...
        for video in videos:
            ...
            if dbvideo == None: raise VideoNotFound
        ...
        for interface in interfaces:
            ...
            if dbinterface == None: raise InterfaceNotFound
        ...
        result = merge(domain, json_user,json_allowed,json_xml,json_disks,json_isos,json_graphics,json_videos,json_interfaces,json_status)
        with app.app_context():
