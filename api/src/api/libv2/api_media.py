...
def domain_from_media(
    user, name, description, icon, create_dict, hyper_pools, disk_size
):
    with app.app_context():
        userObj = (
            r.table("users")
    ...
    dir_disk, disk_filename = get_disk_path(userObj, parsed_name)
    ...
    with app.app_context():
        return _check(r.table("domains").insert(new_domain).run(db.conn), "inserted")
