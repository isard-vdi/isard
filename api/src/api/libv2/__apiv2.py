...
class ApiV2:
    ...
    def UserUpdate(self, user_id, user_name, user_email="", user_photo=""):
        ...
        if not _check(
            r.table("users")
            .get(user_id)
            .update({"name": user_name, "email": user_email, "photo": user_photo})
            .run(db.conn),
            "replaced",
        ):
            raise UpdateFailed
        ...
        try:
            with app.app_context():
                return list(
                    ...
                    .filter({"kind": "desktop"})
                    .order_by("name")
                    .pluck({"id", "name", "icon", "user", "status", "description"})
                    .run(db.conn)
                )
    ...
    def DesktopNewPersistent(
        ...
        with app.app_context():
            try:
                ...
                raise UserNotFound
            if iso != False:
                if r.table("media").get(iso).run(db.conn) == None:
                    raise MediaNotFound
        ...
        if from_template_id == False:
            if disk_size == False:
                if boot == "disk":
                    raise NewDesktopNotBootable
                if boot == "cdrom" and iso == False:
                    raise NewDesktopNotBootable
                hardware["disks"] = []
            else:
                hardware["disks"] = [
                    {"file": dir_disk + "/" + disk_filename, "size": disk_size}
                ]  # 15G as a format   UNITS NEEDED!!!
    ...
    def enrollment_gen(self, role, length=6):
        if role not in ["manager", "advanced", "user"]:
            return False
        ...
        with app.app_context():
            try:
                ...
                template = (
                    r.table("domains")
                    .get(id)
                    .without("xml", "history_domain", "progress")
                    .run(db.conn)
                )
            except:
                raise TemplateNotFound
        ...
        with app.app_context():
            ...
            try:
                return (
                    r.table("categories")
                    .get(category)
                    .run(db.conn)["default_templates"][0]
                )
            except:
                None
        return False
    ...
    def domain_destroy(self, user):
        ...
        with app.app_context():
            r.table("domains").get_all(user, index="user").filter(
                {"status": "Started", "persistent": False}
            ).update({"status": "Stopping"}).run(db.conn)
            r.table("domains").get_all(user, index="user").filter(
                {"status": "Stopped", "persistent": False}
            ).update({"status": "Deleting"}).run(db.conn)
    ...
    def get_domain_id(self, user):
        with app.app_context():
            try:
                return list(
                    r.table("domains").get_all(user, index="user").run(db.conn)
                )[0]["id"]
            except Exception:
                raise
