...
def api_v2_category(id):
    try:
        ...
        return json.dumps({"code":9,"msg":"Register general exception: " + str(e) }), 500, {'Content-Type': 'application/json'}
...
def api_v2_register():
    ...
    try:
        ...
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
        log.error(str(exc_type), str(fname), str(exc_tb.tb_lineno))
        return json.dumps({"code":9,"msg":"Register general exception: " + str(e) }), 401, {'Content-Type': 'application/json'}
...
def api_v2_user_exists(id=False):
    ...
    try:
        ...
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
        log.error(str(exc_type), str(fname), str(exc_tb.tb_lineno))
        return json.dumps({"code":9,"msg":"UserExists general exception: " + str(e) }), 401, {'Content-Type': 'application/json'}
...
def api_v2_user_update(id=False):
    ...
    try:
        ...
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
        log.error(str(exc_type), str(fname), str(exc_tb.tb_lineno))
        return json.dumps({"code":9,"msg":"UserUpdate general exception: " + str(e) }), 401, {'Content-Type': 'application/json'}
...
def api_v2_user_insert():
    ...
    try:
        ...
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
        log.error(str(exc_type), str(fname), str(exc_tb.tb_lineno))
        return json.dumps({"code":9,"msg":"UserNew quota check general exception: " + str(e) }), 401, {'Content-Type': 'application/json'}
...
def api_v2_user_delete(user_id):
    try:
        ...
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
        log.error(str(exc_type), str(fname), str(exc_tb.tb_lineno))
        return json.dumps({"code":9,"msg":"UserDelete general exception: " + str(e) }), 401, {'Content-Type': 'application/json'}
...
def api_v2_user_templates(id=False):
    ...
    try:
        ...
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
        log.error(str(exc_type), str(fname), str(exc_tb.tb_lineno))
        return json.dumps({"code":9,"msg":"UserTemplates general exception: " + str(e) }), 401, {'Content-Type': 'application/json'}
...
def api_v2_user_desktops(id=False):
    ...
    try:
        ...
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
        log.error(str(exc_type), str(fname), str(exc_tb.tb_lineno))
        return json.dumps({"code":9,"msg":"UserDesktops general exception: " + str(e) }), 401, {'Content-Type': 'application/json'}
...
def api_v2_desktop_new():
    ...
    try:
        ...
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
        log.error(str(exc_type), str(fname), str(exc_tb.tb_lineno))
        return json.dumps({"code":9,"msg":"DesktopNew quota check general exception: " + str(e) }), 401, {'Content-Type': 'application/json'}
    ...
    try:
        ...
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
        log.error(str(exc_type), str(fname), str(exc_tb.tb_lineno))
        return json.dumps({"code":9,"msg":"DesktopNew general exception: " + str(e) }), 401, {'Content-Type': 'application/json'}
...
def api_v2_desktop_viewer(desktop_id=False, protocol=False):
    ...
    try:
        ...
        return json.dumps({"code":1,"msg":"Desktop viewer id not found"}), 404, {'Content-Type': 'application/json'}
    except DesktopNotStarted:
        log.error("Viewer for desktop "+desktop_id+" with protocol "+protocol+", desktop not started")
        return json.dumps({"code":2,"msg":"Desktop viewer is not started"}), 404, {'Content-Type': 'application/json'}
    except NotAllowed:
        log.error("Viewer for desktop "+desktop_id+" with protocol "+protocol+", viewer access not allowed")
        return json.dumps({"code":3,"msg":"Desktop viewer id not owned by user"}), 404, {'Content-Type': 'application/json'}
    except ViewerProtocolNotFound:
        log.error("Viewer for desktop "+desktop_id+" with protocol "+protocol+", viewer protocol not found")
        return json.dumps({"code":4,"msg":"Desktop viewer protocol not found"}), 404, {'Content-Type': 'application/json'}
        ...
        return json.dumps({"code":5,"msg":"Desktop viewer protocol not implemented"}), 404, {'Content-Type': 'application/json'}
    except Exception as e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
        log.error(str(exc_type), str(fname), str(exc_tb.tb_lineno))
        return json.dumps({"code":9,"msg":"DesktopViewer general exception: " + str(e) }), 401, {'Content-Type': 'application/json'}
...
def api_v2_desktop_delete(desktop_id=False):
    ...
    try:
        ...
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
        log.error(str(exc_type), str(fname), str(exc_tb.tb_lineno))
        return json.dumps({"code":9,"msg":"DesktopDelete general exception: " + str(e) }), 401, {'Content-Type': 'application/json'}
...
def api_v2_persistent_desktop_new():
    ...
    try:
        ...
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
        log.error(str(exc_type), str(fname), str(exc_tb.tb_lineno))
        return json.dumps({"code":9,"msg":"PersistentDesktopNew quota check general exception: " + str(e) }), 401, {'Content-Type': 'application/json'}
    ...
    ### Needs more!
        ...
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
        log.error(str(exc_type), str(fname), str(exc_tb.tb_lineno))
        return json.dumps({"code":9,"msg":"PersistentDesktopNew general exception: " + str(e) }), 401, {'Content-Type': 'application/json'}
