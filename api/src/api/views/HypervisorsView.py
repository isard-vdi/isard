...
def api_v3_guest_addr():
    ...
    try:
        ...
        if domain_id:
            return json.dumps({"domain_id":domain_id}), 200, {'Content-Type': 'application/json'}
        else:
            log.error("Update guest addr for mac "+mac+" with IP "+ip+", failed!")
            return json.dumps({"code":1,"msg":"UpdateWgAddr update failed"}), 301, {'Content-Type': 'application/json'}
    except Exception as e:
        error = traceback.format_exc()
        log.error("Update guest addr general exception" + error)
        return json.dumps({"code":9,"msg":"Update guest addr general exception: " + error }), 500, {'Content-Type': 'application/json'}
...
def api_v3_hypervisor(hostname=False):
    ...
    if request.method == 'PUT':
        try:
            ...
            if not data['status']:
                log.warning(data)
                return json.dumps({"code":1,"msg":"Failed updating hypervisor: " + data['msg'] }), 301, {'Content-Type': 'application/json'}
            return json.dumps(data['data']), 200, {'Content-Type': 'application/json'}
        except Exception as e:
            error = traceback.format_exc()
            log.error("Hypervisor general exception" + error)
...
def api_v3_hypervisor_vpn(hyp_id):
    try:
        ...
        return json.dumps({"code":9,"msg":"HypervisorVpn general exception: " + error }), 500, {'Content-Type': 'application/json'}
