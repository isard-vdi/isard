...
from .decorators import has_token, is_admin, ownsDomainId, allowedTemplateId
...
def api_v3_template_new(payload):
    ...
    try:
        template_id = templates.New(template_name, 
                                    desktop_id,
    ...
    #     return json.dumps({"code":1,"msg":"TemplateNew user not found"}), 404, {'Content-Type': 'application/json'}
        ...
        return json.dumps({"code":3,"msg":"TemplateNew not created: template id exists"}), 404, {'Content-Type': 'application/json'}
    ### Needs more!
    except Exception as e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
...
def api_v3_template_delete(payload,template_id=False):
    ...
    if not ownsDomainId(payload,template_id): return json.dumps({"code":10,"msg":"Forbidden: "}), 403, {'Content-Type': 'application/json'}
    try:
        templates.Delete(template_id)
        return json.dumps({}), 200, {'Content-Type': 'application/json'}
    except DesktopNotFound:
        log.error("Template delete "+template_id+", template not found")
        return json.dumps({"code":1,"msg":"Template delete id not found"}), 404, {'Content-Type': 'application/json'}
    except DesktopActionFailed:
        log.error("Template delete "+template_id+", template delete failed")
        return json.dumps({"code":5,"msg":"Template delete deleting failed"}), 404, {'Content-Type': 'application/json'}
    except DesktopActionTimeout:
        log.error("Template delete "+template_id+", template delete timeout")
        return json.dumps({"code":6,"msg":"Template delete deleting timeout"}), 404, {'Content-Type': 'application/json'}
    except Exception as e:
        error = traceback.format_exc()
        return json.dumps({"code":9,"msg":"TemplateDelete general exception: " + error }), 401, {'Content-Type': 'application/json'}
