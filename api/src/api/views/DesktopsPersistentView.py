...
def api_v3_desktop_start(payload,desktop_id=False):
    ...
    try:
        ...
        return json.dumps({"code":1,"msg":"DesktopStart user not found"}), 404, {'Content-Type': 'application/json'}
    except Exception as e:
        error = traceback.format_exc()
        return json.dumps({"code":9,"msg":"DesktopStart general exception: " + error }), 401, {'Content-Type': 'application/json'}
    ...
    try:
        ...
        return json.dumps({"code":11,"msg":"DesktopStart user quota CONCURRENT exceeded"}), 507, {'Content-Type': 'application/json'}
    except QuotaGroupConcurrentExceeded:
        log.error("Quota for user "+user_id+" to start a desktop in his group is exceeded")
        return json.dumps({"code":11,"msg":"DesktopStart user limits CONCURRENT exceeded"}), 507, {'Content-Type': 'application/json'}
    except QuotaCategoryConcurrentExceeded:
        log.error("Quota for user "+user_id+" to start a desktop is his category exceeded")
        return json.dumps({"code":11,"msg":"DesktopStart user category limits CONCURRENT exceeded"}), 507, {'Content-Type': 'application/json'}

    except QuotaUserVcpuExceeded:
        log.error("Quota for user "+user_id+" to allocate vCPU is exceeded")
        return json.dumps({"code":11,"msg":"DesktopStart user quota vCPU allocation exceeded"}), 507, {'Content-Type': 'application/json'}
    except QuotaGroupVcpuExceeded:
        log.error("Quota for user "+user_id+" to allocate vCPU in his group is exceeded")
        return json.dumps({"code":11,"msg":"DesktopStart user group limits vCPU allocation exceeded"}), 507, {'Content-Type': 'application/json'}
    except QuotaCategoryVcpuExceeded:
        log.error("Quota for user "+user_id+" to allocate vCPU in his category is exceeded")
        return json.dumps({"code":11,"msg":"DesktopStart user category limits vCPU allocation exceeded"}), 507, {'Content-Type': 'application/json'}

    except QuotaUserMemoryExceeded:
        log.error("Quota for user "+user_id+" to allocate MEMORY is exceeded")
        return json.dumps({"code":11,"msg":"DesktopStart user quota MEMORY allocation exceeded"}), 507, {'Content-Type': 'application/json'}
    except QuotaGroupMemoryExceeded:
        log.error("Quota for user "+user_id+" for creating another desktop is exceeded")
        return json.dumps({"code":11,"msg":"DesktopStart user group limits MEMORY allocation exceeded"}), 507, {'Content-Type': 'application/json'}
    except QuotaCategoryMemoryExceeded:
        log.error("Quota for user "+user_id+" category for desktop MEMORY allocation is exceeded")
        return json.dumps({"code":11,"msg":"DesktopStart user category limits MEMORY allocation exceeded"}), 507, {'Content-Type': 'application/json'}

    except Exception as e:
        error = traceback.format_exc()
        return json.dumps({"code":9,"msg":"DesktopStart quota check general exception: " + error }), 401, {'Content-Type': 'application/json'}
    ...
    try:
        ...
        return json.dumps({"code":2,"msg":"DesktopStart start timeout"}), 408, {'Content-Type': 'application/json'}
    except DesktopActionFailed:
        log.error("Desktop "+desktop_id+" for user "+user_id+" start failed.")
        return json.dumps({"code":3,"msg":"DesktopStart start failed"}), 500, {'Content-Type': 'application/json'}
    except Exception as e:
        error = traceback.format_exc()
        return json.dumps({"code":9,"msg":"DesktopStart general exception: " + error }), 401, {'Content-Type': 'application/json'}
...
def api_v3_desktop_stop(payload,desktop_id=False):
    ...
    try:
        ...
        return json.dumps({"code":1,"msg":"DesktopStop user not found"}), 404, {'Content-Type': 'application/json'}
    except Exception as e:
        error = traceback.format_exc()
        return json.dumps({"code":9,"msg":"DesktopStop general exception: " + error }), 401, {'Content-Type': 'application/json'}
    ...
    try:
        ...
        return json.dumps({"code":2,"msg":"DesktopStop stop timeout"}), 408, {'Content-Type': 'application/json'}
    except DesktopActionFailed:
        log.error("Desktop "+desktop_id+" for user "+user_id+" start failed.")
        return json.dumps({"code":3,"msg":"DesktopStop stop failed"}), 500, {'Content-Type': 'application/json'}
    except Exception as e:
        error = traceback.format_exc()
        return json.dumps({"code":9,"msg":"DesktopStop general exception: " + error }), 401, {'Content-Type': 'application/json'}
...
def api_v3_persistent_desktop_new(payload):
    ...
    try:
        ...
        log.error("Desktop "+desktop_name+" for user "+user_id+" already exists")
        return json.dumps({"code":3,"msg":"PersistentDesktopNew desktop already exists"}), 404, {'Content-Type': 'application/json'}
