...
from ..auth.tokens import get_header_jwt_payload, AuthError

from ..libv2.apiv2_exc import TemplateNotFound, DesktopNotFound
...
def has_token(f):
    ...
    def decorated(*args, **kwargs):
        ...
        raise AuthError({"code": "not_allowed",
                        "description":
                        "Not enough rights"
                        " token."}, 401)
    return decorated
...
def ownsUserId(payload,user_id):
    ...
    if payload['role_id'] == 'manager' and user_id.split['-'][1]==payload['category_id']: return True
    if payload['user_id'] == user_id: return True
    return False

def ownsCategoryId(payload,category_id):
    if payload['role_id'] == 'admin': return True
    if payload['role_id'] == 'manager' and category_id==payload['category_id']: return True
    return False

def ownsDomainId(payload,desktop_id):
    if payload['role_id'] == 'admin': return True
    if payload['role_id'] == 'manager' and payload['category_id'] == desktop_id.split('-')[1]: return True
    if payload['role_id'] == 'advanced':
        with app.app_context():
            if str(r.table('domains').get(desktop_id).pluck('tag').run(db.conn).get('tag',False)).startswith('_'+payload['user_id']):
                return True
    if desktop_id.startswith('_'+payload['user_id']): return True
    return False
...
def allowedTemplateId(payload,template_id):
    try:
        ...
        raise AuthError({"code": 1,
                        "msg":
                        "Not found template "+template_id}, 401)
