...
from .decorators import has_token, is_admin, ownsUserId, ownsCategoryId

@app.route('/api/v3/admin/jwt/<user_id>', methods=['GET'])
@has_token
def api_v3_admin_jwt(payload, user_id):
    if ownsUserId(payload,user_id): return users.Jwt(user_id)
    return json.dumps({"code":10,"msg":"Forbidden: "}), 403, {'Content-Type': 'application/json'}
...
def api_v3_admin_user_exists(payload, id=False):
    ...
    if not ownsUserId(payload,id): return json.dumps({"code":10,"msg":"Forbidden: "}), 403, {'Content-Type': 'application/json'}
    try:
...
def api_v3_admin_users(payload):
    try:
        ...
        return json.dumps({"code":9,"msg":"Users list general exception: " + traceback.format_exc() }), 401, {'Content-Type': 'application/json'}

    if payload['role_id'] == 'admin': return json.dumps(userslist), 200, {'Content-Type': 'application/json'}
    if payload['role_id'] == 'manager':
        filtered_users=[u for u in userslit if u['category'] == payload['category_id']]
...
def api_v3_admin_user_insert(payload):
    ...
    if not ownsCategoryId(payload,category_id): return json.dumps({"code":10,"msg":"Forbidden: "}), 403, {'Content-Type': 'application/json'}
    try:
...
def api_v3_admin_user_delete(payload, user_id):
    ...
    if not ownsUserId(payload,user_id): return json.dumps({"code":10,"msg":"Forbidden: "}), 403, {'Content-Type': 'application/json'}
    try:
...
def api_v3_admin_category_delete(category_id,payload):
    try:
        return json.dumps(users.CategoryDelete(category_id)), 200, {'Content-Type': 'application/json'}
    except:
        log.error(traceback.format_exc())
        return json.dumps({"code":9,"msg":"CategoryDelete general exception: " + traceback.format_exc() }), 401, {'Content-Type': 'application/json'}


@app.route('/api/v3/admin/groups', methods=['GET'])
@has_token
def api_v3_admin_groups(payload):
    if payload['role_id'] not in ['admin','manager']: return json.dumps({"code":10,"msg":"Forbidden: "}), 403, {'Content-Type': 'application/json'}
    groups=users.GroupsGet()
    if payload['role_id'] == 'admin': return json.dumps(groups), 200, {'Content-Type': 'application/json'} 
    try:
        filtered_groups = [g for g in groups if g['parent_category']==payload['category_id']]
        return json.dumps(filtered_groups), 200, {'Content-Type': 'application/json'}
    except:
        log.error(traceback.format_exc())
        return json.dumps({"code":9,"msg":"GroupsGet general exception: " + traceback.format_exc() }), 401, {'Content-Type': 'application/json'}

@app.route('/api/v3/admin/group/<group_id>', methods=['DELETE'])
@is_admin
def api_v3_admin_group_delete(group_id,payload):
    try:
        return json.dumps(users.GroupDelete(group_id)), 200, {'Content-Type': 'application/json'}
    except:
        log.error(traceback.format_exc())
        return json.dumps({"code":9,"msg":"GroupDelete general exception: " + traceback.format_exc() }), 401, {'Content-Type': 'application/json'}
