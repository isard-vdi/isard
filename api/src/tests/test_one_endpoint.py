...
raw_jwt_data={  'exp': datetime.utcnow() + timedelta(minutes=5),
                        'kid':admin_secret_data['id'],
                        'data':{
                            'role_id':admin_secret_data['role_id'],
                            'category_id':admin_secret_data['category_id']}}
admin_jwt=jwt.encode(raw_jwt_data,
                        admin_secret_data['secret'],
                        algorithm='HS256')
auths['isardvdi']={'secret':admin_secret_data,
                    'jwt': admin_jwt,
                    'header':{'Authorization': 'Bearer ' + admin_jwt}}

pprint(raw_jwt_data)

data = {'name': 'My new desktop',
        'disk_user': True,
        'virt_install_id': 'win10Virtio'
        }
## Warning, second time will throw DesktopExists exception as the name has been used already

response = requests.post(base+'/desktop/from/scratch',
                        data=data,
