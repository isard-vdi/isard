#!/bin/bash
#
# Redis Borg Statistics Reporter
# Outputs detailed statistics for the just-completed redis borg backup
#

# Get backup configuration from environment
BACKUP_WHEN="${BACKUP_REDIS_WHEN:-disabled}"
...
if [ ! -d "$REPO_PATH" ]; then
    echo "$(date '+%b %d %H:%M:%S') Warning: Redis borg repository not found at $REPO_PATH"
    exit 0
fi
