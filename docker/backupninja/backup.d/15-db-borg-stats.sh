#!/bin/bash
#
# Database Borg Statistics Reporter
# Outputs detailed statistics for the just-completed database borg backup
#

# Get backup configuration from environment
BACKUP_WHEN="${BACKUP_DB_WHEN:-disabled}"
...
if [ ! -d "$REPO_PATH" ]; then
    echo "$(date '+%b %d %H:%M:%S') Warning: Database borg repository not found at $REPO_PATH"
    exit 0
fi
...
echo "$(date '+%b %d %H:%M:%S') Info: $LOG_PREFIX: Collecting statistics for database backup"
