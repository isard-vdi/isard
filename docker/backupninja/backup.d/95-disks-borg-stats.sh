#!/bin/bash
#
# Disks Borg Statistics Reporter
# Outputs detailed statistics for the just-completed disks borg backup
#

# Get backup configuration from environment
BACKUP_WHEN="${BACKUP_DISKS_WHEN:-disabled}"
...
REPO_PATH="/backup/disks"
LOG_PREFIX="BORG_STATS_DISKS"
...
if [ ! -d "$REPO_PATH" ]; then
    echo "$(date '+%b %d %H:%M:%S') Warning: Disks borg repository not found at $REPO_PATH"
    exit 0
fi
...
echo "$(date '+%b %d %H:%M:%S') Info: $LOG_PREFIX: Collecting statistics for disks backup"
