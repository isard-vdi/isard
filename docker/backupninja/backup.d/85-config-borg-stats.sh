#!/bin/bash
#
# Config Borg Statistics Reporter
# Outputs detailed statistics for the just-completed config borg backup
#

# Get backup configuration from environment
BACKUP_WHEN="${BACKUP_CONFIG_WHEN:-disabled}"
...
REPO_PATH="/backup/config"
LOG_PREFIX="BORG_STATS_CONFIG"
...
if [ ! -d "$REPO_PATH" ]; then
    echo "$(date '+%b %d %H:%M:%S') Warning: Config borg repository not found at $REPO_PATH"
    exit 0
fi
...
echo "$(date '+%b %d %H:%M:%S') Info: $LOG_PREFIX: Collecting statistics for config backup"
