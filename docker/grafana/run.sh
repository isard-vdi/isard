...
if [ ! -z "$GRAFANA_URL" ]; then
        sed -i "/^domain/c\domain = $GRAFANA_URL" /grafana/conf/defaults.ini
        sed -i "/^root_url/c\root_url = https://$GRAFANA_URL:$GRAFANA_PORT$GRAFANA_PATH" /grafana/conf/defaults.ini
