    ...
    def apply_remote_vpn(self,user,desktop_ip):
        for extra_alloweds in self.get_extra_alloweds(user):
            ...
            if extra_alloweds['vpn']['wireguard']['extra_client_nets']:
                for extra_addr in extra_alloweds['vpn']['wireguard']['extra_client_nets'].split(','):
                    check_output(
                        (
                            '/sbin/iptables',
                            '-A',
    ...
    def remove_matching_rules(self,peer):
        try:
            if peer['vpn']['wireguard']['extra_client_nets']:
                ips=peer['vpn']['wireguard']['extra_client_nets'].split(',')+[peer['vpn']['wireguard']['Address']]
            else:
                ips=[peer['vpn']['wireguard']['Address']]
            rules=iptc.easy.dump_table('filter')['FORWARD']
            for rule in rules:
                for ip in ips:
                    if rule['dst'].split('/')[0] in ip or rule['src'].split('/')[0] in ips:
                        try:
                            check_output(
                                (
                                    '/sbin/iptables',
