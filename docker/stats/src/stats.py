...
GRACE_NOVIEWER_SHUTDOWN_TIME = int(
    os.environ.get("STATS_GRACE_NOVIEWER_SHUTDOWN_TIME", 1800)
)
...
class HypStats:
    ...
    def __init__(
        ...
        if send_to_influx is True:
            ...
            ### wait for influx
            while True:
                self.client_influxdb = InfluxDBClient(
                ...
                if health.status == "pass":
                    ...
                    print(
                        f"error with influxdb connection with status {health.status}: {health.message}"
                    )
                    print(f"retry connect to influxdb in {INFLUX_RETRY_PERIOD}s")
                    time.sleep(INFLUX_RETRY_PERIOD)

        self.grace_noviewer_shutdown_time = os.environ.get(
            "GRACE_NOVIEWER_SHUTDOWN_TIME", GRACE_NOVIEWER_SHUTDOWN_TIME
        )
    ...
    def wait_to_libvirt_connection_is_alive(self):
        ...
        while self.conn is False:
            if self.open_libvirt_connection() is False:
                print(
                    f"waiting {self.interval_stats} seconds to try connect to libvirt hypervisor"
                )
    ...
    def open_libvirt_connection(self):
        if self.hyp_hostname == "localhost":
            ...
            uri = "qemu+ssh://{}@{}:{}/system".format(
                self.hyp_user, self.hyp_hostname, self.hyp_port
            )
        try:
    ...
    def stats_loop(self):
        while self.stop is False:
            ...
            next_iteration = (
                int(time_end_iteration)
                - int(time_end_iteration) % int(self.interval_stats)
                + int(self.interval_stats)
            )
    ...
    def launch_actions_in_iterations(self):
        ...
        print(
            "PHASE 1 - {}: extract libvirt stats ({} seconds)".format(
                "ok" if result is True else "ko", round(time.time() - start_time, 3)
            )
        )
        if self.total_domains > 0:
            ...
            print(
                "PHASE 2 - {}: extract socket counters ({} seconds)".format(
                    "ok" if result is True else "ko", round(time.time() - start_time, 3)
                )
            )
            ...
            print(
                "PHASE 3 - {}: process domain info ({} seconds)".format(
                    "ok" if result is True else "ko", round(time.time() - start_time, 3)
                )
            )
            ...
            if phases[3] is True and phases[2] is True:
                ...
                print(
                    "PHASE 4 - {}: process viewers info ({} seconds)".format(
                        "ok" if result is True else "ko",
            ...
            if phases[2] is True and phases[3] is True and phases[4] is True:
                ...
                print(
                    "PHASE 5 - {}: process stats ({} seconds)".format(
                        "ok" if result is True else "ko",
                        round(time.time() - start_time, 3),
                    )
                )
        ...
        print(
            "PHASE 6 - {}: process stats ({} seconds)".format(
                "ok" if result is True else "ko", round(time.time() - start_time, 3)
            )
        )
    ...
    def send_to_influx(self, start_time):
        ...
        try:
            flatted_hyp_stats = flatten(
                {
                    "cpu": self.out_raw["cpu"],
            ...
            if self.total_domains > 0:
                ...
                for id_domain, d_stats in self.processed_stats.items():
                    ...
                    flatted = flatten(
                        stats, reducer="underscore", enumerate_types=(list,)
                    )
                    data.append(
                        {
                            "measurement": "domains",
        ...
        for domain, d_stats in self.out_raw["domains_stats"]:
            try:
                ...
                if "net" in unflatted.keys():
                    ...
                    d_processed_stats["nets"]["tx_bytes"] = sum(
                        [
                            unflatted["net"][str(i)]["tx"]["bytes"]
                            for i in range(unflatted["net"]["count"])
                        ]
                    )
                    d_processed_stats["nets"]["rx_bytes"] = sum(
                        [
                            unflatted["net"][str(i)]["rx"]["bytes"]
                            for i in range(unflatted["net"]["count"])
                        ]
                    )
                    d_processed_stats["nets"]["tx_pkts"] = sum(
                        [
                            unflatted["net"][str(i)]["tx"]["pkts"]
                            for i in range(unflatted["net"]["count"])
                        ]
                    )
                    d_processed_stats["nets"]["rx_pkts"] = sum(
                        [
                            unflatted["net"][str(i)]["rx"]["pkts"]
                            for i in range(unflatted["net"]["count"])
                        ]
                    )
                    d_processed_stats["nets"]["tx_errs"] = sum(
                        [
                            unflatted["net"][str(i)]["tx"]["errs"]
                            for i in range(unflatted["net"]["count"])
                        ]
                    )
                    d_processed_stats["nets"]["rx_errs"] = sum(
                        [
                            unflatted["net"][str(i)]["rx"]["errs"]
                            for i in range(unflatted["net"]["count"])
                        ]
                    )
                    d_processed_stats["nets"]["tx_drop"] = sum(
                        [
                            unflatted["net"][str(i)]["tx"]["drop"]
                            for i in range(unflatted["net"]["count"])
                        ]
                    )
                    d_processed_stats["nets"]["rx_drop"] = sum(
                        [
                            unflatted["net"][str(i)]["rx"]["drop"]
                            for i in range(unflatted["net"]["count"])
                        ]
                    )
                ...
                d_processed_stats["vcpus"]["cpu_time"] = {
                    i: unflatted["vcpu"][str(i)]["time"]
                    for i in range(unflatted["vcpu"]["current"])
                }
                d_processed_stats["vcpus"]["sum_cpu_time"] = sum(
                    [
                        unflatted["vcpu"][str(i)]["time"]
                        for i in range(unflatted["vcpu"]["current"])
                    ]
                )
                d_processed_stats["vcpus"]["cpu_wait"] = {
                    i: unflatted["vcpu"][str(i)]["wait"]
                    for i in range(unflatted["vcpu"]["current"])
                }
                d_processed_stats["vcpus"]["sum_cpu_wait"] = sum(
                    [
                        unflatted["vcpu"][str(i)]["wait"]
                        for i in range(unflatted["vcpu"]["current"])
                    ]
                )
                ...
                if "block" in unflatted.keys():
                    ...
                    d_processed_stats["blocks"]["rd_bytes"] = sum(
                        [
                            unflatted["block"][str(i)]["rd"]["bytes"]
                            for i in range(unflatted["block"]["count"])
                        ]
                    )
                    d_processed_stats["blocks"]["wr_bytes"] = sum(
                        [
                            unflatted["block"][str(i)]["wr"]["bytes"]
                            for i in range(unflatted["block"]["count"])
                        ]
                    )
                    d_processed_stats["blocks"]["rd_reqs"] = sum(
                        [
                            unflatted["block"][str(i)]["rd"]["reqs"]
                            for i in range(unflatted["block"]["count"])
                        ]
                    )
                    d_processed_stats["blocks"]["wr_reqs"] = sum(
                        [
                            unflatted["block"][str(i)]["wr"]["reqs"]
                            for i in range(unflatted["block"]["count"])
                        ]
                    )
                    d_processed_stats["blocks"]["fl_reqs"] = sum(
                        [
                            unflatted["block"][str(i)]["fl"]["reqs"]
                            for i in range(unflatted["block"]["count"])
                        ]
                    )
                    d_processed_stats["blocks"]["rd_times"] = sum(
                        [
                            round(
                                unflatted["block"][str(i)]["rd"]["times"] / 1000000000,
                                3,
                            )
                            for i in range(unflatted["block"]["count"])
                        ]
                    )
                    d_processed_stats["blocks"]["wr_times"] = sum(
                        [
                            round(
                                unflatted["block"][str(i)]["wr"]["times"] / 1000000000,
                                3,
                            )
                            for i in range(unflatted["block"]["count"])
                        ]
                    )
                    d_processed_stats["blocks"]["fl_times"] = sum(
                        [
                            round(
                                unflatted["block"][str(i)]["fl"]["times"] / 1000000000,
                                3,
                            )
                            for i in range(unflatted["block"]["count"])
                        ]
                    )
                    d_processed_stats["blocks"]["allocation"] = sum(
                        [
                            unflatted["block"][str(i)]["allocation"]
                            for i in range(unflatted["block"]["count"])
                            if unflatted["block"][str(i)]["path"]
                            .split("/")[-1]
                            .find("qcow")
                            > 0
                        ]
                    ...
                    d_processed_stats["blocks"]["capacity"] = sum(
                        [
                            unflatted["block"][str(i)]["allocation"]
                            for i in range(unflatted["block"]["count"])
                            if unflatted["block"][str(i)]["path"]
                    ...
                    d_processed_stats["blocks"]["physical"] = sum(
                        [
                            unflatted["block"][str(i)]["allocation"]
                            for i in range(unflatted["block"]["count"])
                            if unflatted["block"][str(i)]["path"]
    ...
    def process_socket_info(self):
        start_time = time.time()
        try:
            self.d_domains_viewers_stats = translate_socket_stats_to_domains(
                self.domain_info["d_domains_info"], self.d_socket_by_port
            )
        except Exception as e:
        ...
        try:
            ...
            set_domains_with_viewers = set(
                [
                    k
                    for k, v in self.d_domains_viewers_stats.items()
                    if v["total_connections"] > 0
                ]
            )
            set_domains_with_no_viewers = (
                set(self.d_domains_viewers_stats.keys()) - set_domains_with_viewers
            )

            # identify new domains with viewers and domains that have disconnect viewers
            set_previous_domains_with_viewers = set(
                self.now_domains_with_viewers.keys()
            )
            set_viewers_switch_off = (
                set_previous_domains_with_viewers - set_domains_with_viewers
            )
            set_viewers_switch_on = (
                set_domains_with_viewers - set_previous_domains_with_viewers
            )
            ...
            for k in set_domains_with_viewers:
                if k not in self.now_domains_with_viewers.keys():
                    self.now_domains_with_viewers[k] = {
                        "timestamp_viewer_on": start_time
                    }
            ...
            for k in set_viewers_switch_off:
                ...
                self.domains_noviewer_grace_time[k] = {
                    "timestamp_viewer_off": start_time
                }
            ...
            for k, v in self.domains_noviewer_grace_time.items():
                if v["timestamp_viewer_off"] < (
                    start_time - self.grace_noviewer_shutdown_time
                ):
                    self.domains_noviewer_nograce_time[k] = {
                        "timestamp_to_shutdown": start_time
                    }
    ...
    def process_domain_info(self):
        try:
            ...
            (
                ...
                self.domain_info["d_domains_xml"],
            ) = domain_stats_as_dicts(self.out_raw["domains_stats"])
            return True
        except Exception as e:
            self.domain_info = False
    ...
    def extract_socket_stats(self):
        start_time = time.time()
        try:
            self.d_socket_by_port = get_socket_stats(
                type_conn_ss=self.type_conn_ss,
    ...
    def extract_stats_libvirt(self):
        ...
        try:
            ...
            if len(list_all_domains) > 0:
                ...
                if (
                    self.max_domains_bulk_extract == 0
                    or len(list_all_domains) <= self.max_domains_bulk_extract
                ):
                    ...
                    next_domains_to_extract = [
                        d for d in list_all_domains if d.ID() > self.last_domain_id
                    ][: self.max_domains_bulk_extract]
                    if len(next_domains_to_extract) < self.max_domains_bulk_extract:
                        we_need_more_domains = self.max_domains_bulk_extract - len(
                            next_domains_to_extract
                        )
                        next_domains_to_extract += list_all_domains[
                            :we_need_more_domains
                        ]
                    # EXTRACT STATS FROM DOMAINS IN LIST
                    out_domains_raw = self.conn.domainListGetStats(
                        next_domains_to_extract
                    )
