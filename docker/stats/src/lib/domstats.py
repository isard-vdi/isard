...
def domstats_libvirt(uri=None):
    ...
    out_all_domains_raw = conn.getAllDomainStats(
        ...
        if xml is not False:
            ...
            try:
                if type(d_xml["domain"]["devices"]["graphics"]) is list:
                    d_graphics = {
                        d["@type"]: d for d in d_xml["domain"]["devices"]["graphics"]
                    }
                else:
                    d_graphics = {
                        d_xml["domain"]["devices"]["graphics"]["@type"]: d_xml[
                            "domain"
                        ]["devices"]["graphics"]
                    }
                d_domain_info["port_spice_tls"] = int(
                    d_graphics.get("spice", {}).get("@tlsPort", 0)
                )
                d_domain_info["port_spice"] = int(
                    d_graphics.get("spice", {}).get("@port", 0)
                )
                d_domain_info["port_vnc"] = int(
                    d_graphics.get("vnc", {}).get("@port", 0)
                )
            except:
...
def translate_socket_stats_to_domains(d_domains_info, d_socket_by_port):
    ...
    for id_domain, d_domain_info in d_domains_info.items():
        ...
        if d_domain_info["port_spice_tls"] in d_socket_by_port.keys():
            ...
            d_domains_viewers_stats[id_domain]["spice_bytes_sent"] += d_socket_by_port[
                port
            ]["bytes_sent"]
            d_domains_viewers_stats[id_domain]["total_bytes_sent"] += d_socket_by_port[
                port
            ]["bytes_sent"]
            d_domains_viewers_stats[id_domain][
                "spice_bytes_received"
            ] += d_socket_by_port[port]["bytes_received"]
            d_domains_viewers_stats[id_domain][
            ...
            d_domains_viewers_stats[id_domain]["total_sockets"] += len(
                d_socket_by_port[port]["dports"]
            )
        ...
        if d_domain_info["port_vnc"] in d_socket_by_port.keys():
            ...
            d_domains_viewers_stats[id_domain]["vnc_connections"] += len(
                d_socket_by_port[port]["dports"]
            )
            d_domains_viewers_stats[id_domain]["total_connections"] += 1
            d_domains_viewers_stats[id_domain]["vnc_bytes_sent"] += d_socket_by_port[
                port
            ]["bytes_sent"]
            d_domains_viewers_stats[id_domain]["total_bytes_sent"] += d_socket_by_port[
                port
