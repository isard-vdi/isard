...
from lib.domstats import (
    domain_stats_as_dicts,
    domstats_libvirt,
    domstats_with_virsh_docker_exec,
)
...
for j in range(6):
    ...
    for i in range(20):
        ...
        d_stats, d_domains_info, d_domains_objects, d_xml = domain_stats_as_dicts(
            raw_stats_domains
        )
...
class StatsHyper:
    ...
    def process_domains_stats(self, raw_stats_domains):
        ...
        for d in add_domains:
            ...
            self.stats_raw_domains[d] = deque(
                maxlen=self.stats_queue_lenght_domains_raw_stats
            )
        ...
        for d, raw in d_all_domain_stats.items():
            ...
            raw["stats"]["now_datetime"] = datetime.utcfromtimestamp(
                raw_stats["time_utc"]
            )
            ...
            if len(self.stats_raw_domains[d]) > 1:
                ...
                if delta == 0:
                    log.error(
                        "same value in now_utc_time, must call get_stats_from_libvirt"
                    )
                    break
                ...
                if current.get("cpu.time") and previous.get("cpu.time"):
                    d_stats["cpu_load"] = round(
                        (current["cpu.time"] - previous["cpu.time"])
                        / 1000000000
                        / self.info["cpu_threads"],
                        3,
                    )

                d_balloon = {
                    k: v / 1024 for k, v in current.items() if k[:5] == "ballo"
                }

                # balloon is running and monitorized if balloon.unused key is disposable
                if "balloon.unused" in d_balloon.keys():
                    mem_used = round(
                        (d_balloon["balloon.current"] - d_balloon["balloon.unused"])
                        / 1024.0,
                        3,
                    )
                ...
                d_stats["mem_load"] = round(
                    mem_used / (self.info["memory_in_MB"] / 1024), 2
                )
                ...
                for n in range(current["vcpu.current"]):
                    try:
                        vcpu_total_time += (
                            current["vcpu." + str(n) + ".time"]
                            - previous["vcpu." + str(n) + ".time"]
                        )
                        vcpu_total_wait += (
                            current["vcpu." + str(n) + ".wait"]
                            - previous["vcpu." + str(n) + ".wait"]
                        )
                    except KeyError:
                        vcpu_total_time = 0
                        vcpu_total_wait = 0

                d_stats["vcpu_load"] = round(
                    (vcpu_total_time / (delta * 1e9) / current["vcpu.current"]) * 100, 2
                )
                d_stats["vcpu_iowait"] = round(
                    (vcpu_total_wait / (delta * 1e9) / current["vcpu.current"]) * 100, 2
                )
                ...
                if "block.count" in current.keys():
                    for n in range(current["block.count"]):
                        try:
                            total_block_wr += (
                                current["block." + str(n) + ".wr.bytes"]
                                - previous["block." + str(n) + ".wr.bytes"]
                            )
                            total_block_rd += (
                                current["block." + str(n) + ".rd.bytes"]
                                - previous["block." + str(n) + ".rd.bytes"]
                            )
                            total_block_wr_reqs += (
                                current["block." + str(n) + ".wr.reqs"]
                                - previous["block." + str(n) + ".wr.reqs"]
                            )
                            total_block_rd_reqs += (
                                current["block." + str(n) + ".rd.reqs"]
                                - previous["block." + str(n) + ".rd.reqs"]
                            )
                        except KeyError:
                            total_block_wr = 0
                            total_block_wr_reqs = 0
                ...
                if "net.count" in current.keys():
                    for n in range(current["net.count"]):
                        try:
                            total_net_tx += (
                                current["net." + str(n) + ".tx.bytes"]
                                - previous["net." + str(n) + ".tx.bytes"]
                            )
                            total_net_rx += (
                                current["net." + str(n) + ".rx.bytes"]
                                - previous["net." + str(n) + ".rx.bytes"]
                            )
                        except KeyError:
                            total_net_tx = 0
                            total_net_rx = 0
        ...
        if len(self.stats_domains_now) > 0:
            try:
                mean_vcpu_load = sum(
                    [j["vcpu_load"] for j in self.stats_domains_now.values()]
                ) / len(self.stats_domains_now)
                mean_vcpu_iowait = sum(
                    [j["vcpu_iowait"] for j in self.stats_domains_now.values()]
                ) / len(self.stats_domains_now)
            except KeyError:
