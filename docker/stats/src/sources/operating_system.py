...
class OperatingSystem():
    ...
    def stats(self):
        while True:
            ...
            cpu = { "cpu.freqcur": str(cpufreq.current), #self.cpu['freqcur'], 
                    "cpu.loadperc":str(psutil.cpu_percent())}
            c.send2carbon({"system":cpu})

            svmem = psutil.virtual_memory()
            mem = { "mem.total":str(svmem.total),
    ...
    def one_shot(self):
        cpu=self.parse_cpu()
        cpufreq = psutil.cpu_freq()
        cpu = { "cpu.sockets": cpu['sockets'],
                "cpu.cores": cpu['cores'], #str(psutil.cpu_count(logical=False)),
                "cpu.threads": cpu['threads'], #str(psutil.cpu_count(logical=True)-psutil.cpu_count(logical=False)),
                "cpu.totalth": str(int(cpu['sockets'])*int(cpu['cores'])*int(cpu['threads'])),
                "cpu.freqmax": cpu['freqmax'], #str(cpufreq.max),
                "cpu.freqmin": cpu['freqmin']} #str(cpufreq.min),
        c.send2carbon({"system":cpu})

    def parse_cpu(self):
        cpu={}
        for line in execute(["lscpu"]):
            if line.startswith("Thread"): cpu["threads"]=line.split(":")[1].strip()
            if line.startswith("Core"): cpu["cores"]=line.split(":")[1].strip()
            if line.startswith("Socket"): cpu["sockets"]=line.split(":")[1].strip()
            if line.startswith("CPU MHz"): cpu["freqcur"]=line.split(":")[1].strip()
            if line.startswith("CPU max"): cpu["freqmax"]=line.split(":")[1].strip()
            if line.startswith("CPU min"): cpu["freqmin"]=line.split(":")[1].strip()
        return cpu