...
class DomainsEvents:
    def __init__(
        self, uri="qemu+unix:///system?socket=/var/run/libvirt/libvirt-sock-ro"
    ):
        self.uri = uri
        self.thread = None
        # ~ self.dom_started={}
    ...
    def event_graphics_cb(
        self, conn, dom, phase, localAddr, remoteAddr, authScheme, subject, opaque
    ):
        if dom.name() not in self.viewer.keys():
            self.viewer[dom.name()] = {
                "spice-client": {
                    "start": 0,
                    "stop": 0,
                    "remote": remoteAddr,
                ...
                c.send2carbon(
                    {
                        ...
                        + viewer: {
                            "use": str(
                                round(
                                    self.viewer[dom.name()][viewer]["stop"]
                                    - self.viewer[dom.name()][viewer]["start"]
                                )
                            )
                        }
                    }
