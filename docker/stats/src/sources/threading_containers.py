...
class Containers:
    ...
    def stats(self):
        ...
        processes = [
            mp.Process(target=self.process_stats, args=(i.name, i.stats(stream=False)))
    ...
    def process_stats(name, stats):
        ...
        try:
            for net in stats["networks"].keys():
                docks[name + ".net." + net + ".rx_bytes"] = str(
                    stats["networks"][net]["rx_bytes"]
                )
                docks[name + ".net." + net + ".tx_bytes"] = str(
                    stats["networks"][net]["tx_bytes"]
                )
        except:
            None
