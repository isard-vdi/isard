...
def process_items(dir_name,suffixes,endpoint):
    ...
    list_of_files = {str(p.resolve()) for p in Path(dir_name).glob("**/*") if p.suffix in size_suffixes}
...
def process_item(file_path):
    ...
    if file_path.startswith(BASE_PATH+'/media'):
        if '.'+file_path.split('.')[-1] not in media_ext: #+compress_ext:
            print('MEDIA ERROR: The file '+file_path+' does not match extensions: '+str(media_ext))
            exit(1)
    elif file_path.startswith(BASE_PATH+'/groups'):
        if '.'+file_path.split('.')[-1] not in disk_ext: #+compress_ext:
            print('DISKS ERROR: The file '+file_path+' does not match extensions: '+str(disk_ext))
            exit(1)
    else:
        print('Can only process media and groups folders. You tried to add: '+file_path)
        exit(1)

    # uncompress(file_path)
    endpoint='hypervisor/media_found' if file_path.startswith(BASE_PATH+'/media') else 'hypervisor/disks_found'
...
def delete_item(file_path):
    ...
    if file_path.startswith(BASE_PATH+'/media'):
        ...
        print('Can only delete media and groups folders. You tried to add: '+file_path)
        exit(1)
        ...
        if file_path.endswith('.tar.gz'):
            with tarfile.open(file_path) as tar:
                for tf in tar.getnames():
                    if tf.startswith('media/') and tf.split('.')[-1] in media_ext: continue
                    if tf.startswith('groups/') and tf.split('.')[-1] in disk_ext: continue
                    if tf.startswith('templates/') and tf.split('.')[-1] in disk_ext: continue
...
if __name__ == '__main__':
    ...
    if not len(myargs) or '-h' in myargs or '--help' in myargs:
        ...
        print('   -m/--media                Will scan all media path an add missing to media.')
        print('   -g/--groups               Will scan all groups path an add missing to media.')
        print('   -a/--all                  Will scan media and groups paths an add missing to media.')
    ...
    if '-g' in myargs or '--groups' in myargs:
        process_items(BASE_PATH+'/groups',[".qcow", ".qcow2"],'hypervisor/disks_found')
