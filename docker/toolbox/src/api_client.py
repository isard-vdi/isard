...
class ApiClient():
    ...
    def post(self,url,data={}):
        try:
            ...
            resp = requests.post(self.base_url+url, 
                                data=data, 
    ...
    def delete(self,url):
        try:
            resp = requests.delete(self.base_url+url, 
                                headers=self.header_auth(), 
                                verify=self.verifycert)
            if resp.status_code == 200:
                return json.loads(resp.text)
    ...
    def update(self,url,data={}):
        try:
            ...
            resp = requests.put(self.base_url+url, 
                                data=data, 
                                headers=self.header_auth(), 
                                verify=self.verifycert)
            if resp.status_code == 200:
    ...
    def header_auth(self):
        token=jwt.encode({  'exp': datetime.utcnow() + timedelta(seconds=20),
                                'kid': 'isardvdi-hypervisors',
                                'data':{
                                    'id': 'isard-hypervisor' if os.environ['HOSTNAME'] == 'localhost' else os.environ['HOSTNAME']
                                    }},
