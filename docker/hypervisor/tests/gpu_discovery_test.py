...
Run locally:
    cd docker/hypervisor && python -m pytest tests/gpu_discovery_test.py -v
"""
...
import pytest
from isardvdi_hypervisor.gpu_discovery import (
...
def test_framebuffer_mb_from_suffix(suffix, expected_mb):
    from isardvdi_hypervisor.gpu_discovery import _framebuffer_mb_from_suffix

    assert _framebuffer_mb_from_suffix(suffix) == expected_mb


def test_parse_creatable_vgpu_types_parses_id_name_lines():
    from isardvdi_hypervisor.gpu_discovery import _parse_creatable_vgpu_types
...
def test_get_vgpu_profiles_vfio_reads_creatable_types(monkeypatch, tmp_path):
    from isardvdi_hypervisor.gpu_discovery import _get_vgpu_profiles_vfio
...
def test_check_gpu_tooling_errors_when_sriov_manage_missing(monkeypatch, caplog):
    ...
    with caplog.at_level(logging.ERROR, logger="isardvdi_hypervisor.gpu_discovery"):
...
def test_check_gpu_tooling_silent_when_tools_present(monkeypatch, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="isardvdi_hypervisor.gpu_discovery"):
        gd._check_gpu_tooling()
...
def test_annotate_mig_backed_vgpu_profiles_maps_slice_to_gfx_gi():
    from isardvdi_hypervisor import gpu_discovery as _gd
