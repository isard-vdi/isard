...
gpu_apply loads the shared builders from /src/isardvdi_common/lib (container) or
the repo path (here). The apply orchestration is driven through a STATEFUL host
fake whose run() applies the driver/mdev side effects the real sysfs writes
would, so the post-apply readback gate, VF-carving and post-prep type resolution
are genuinely exercised (the old fakes masked those by returning
fixed/unreachable states).
"""

import os

import pytest
from isardvdi_hypervisor import gpu_apply as ga
