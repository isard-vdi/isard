...
Loads the shared ``gpu_pool_policy`` via the same ``__file__``-relative fallback
``gpu_apply`` uses, so it resolves under both the container layout
(``/src/isardvdi_common/lib``) and a repo checkout.
"""
