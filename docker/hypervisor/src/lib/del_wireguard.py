...
def SetupWireguard(hypervisor):
    ...
    wireguard_xml = """<network xmlns:dnsmasq='http://libvirt.org/schemas/network/dnsmasq/1.0'> \
      ...
      <dns enable="no"/> \
        ...
        dhcp_subnet_gw,
        dhcp_subnet_prefix,
        dhcp_subnet_range_start,
        dhcp_subnet_range_end,
        main_net,
        dhcp_subnet_gw,
    )
    ...
    print(
        "Setting wireguard-hypervisor internal net interface IP: ip a a "
        + str(list(int_net.hosts())[2])
        + "/"
        + str(int_net.prefixlen)
        + " dev "
        + os.environ["WG_INTERFACE"]
    )
    os.system(
        "ip a a "
        + str(list(int_net.hosts())[2])
    ...
    print(
        "Setting route to wireguard users network: ip r a "
        + main_net
        + " via "
        + str(list(int_net.hosts())[1])
    )
