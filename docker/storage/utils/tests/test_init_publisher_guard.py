...
``REDIS_WORKERS=0`` says this node runs no storage worker, so it holds no pool:
every pool path resolves to whatever filesystem contains it, which is the root
disk. Published under the pool's key that figure is not merely useless -- the
first writer owns the key for its whole TTL, so the node that does hold the
pool cannot correct it, and every free-space decision reads the root disk of a
machine that was never storing anything.
...
def _block():
    """The publishing decision, from its banner to the end of its if."""
...
def test_the_guard_reads_the_same_variable_the_workers_do():
    ...
    block = _block()
    assert re.search(r"\$\{REDIS_WORKERS:-1\}", block)
