...
class isardViewer:
    ...
    def get_viewer(self, data, current_user, remote_addr=False):
        ...
        if "preferred" in data.keys():
            ...
            r.table("domains").get(data["pk"]).update(
                {"options": {"viewers": domain["options"]["viewers"]}}
            ).run(db.conn)
        ...
        if current_user.role == "admin":
            ...
            id = data["pk"]
            if id.startswith("_" + current_user.id + "_") or id.startswith(
                "_disposable_" + remote_addr.replace(".", "_") + "_"
            ):
                return self.send_viewer(data, domain, remote_addr=remote_addr)
    ...
    def send_viewer(self, data, domain, kind="domain", remote_addr=False):
        if data["kind"] == "spice-html5":
            viewer = self.get_domain_spice_data(
                data["pk"], domain, remote_addr=remote_addr
            )
        ...
        if data["kind"] == "vnc-html5":
            viewer = self.get_domain_vnc_data(
                data["pk"], domain, remote_addr=remote_addr
            )
            if viewer is not False:
        ...
        if data["kind"] == "vnc-client":
            ...
            consola = self.get_vnc_file(
                viewer, data["pk"], remote_addr=remote_addr, os=data["os"]
            )
            return {
                "kind": "file",
                "ext": consola[0],
    ...
    def get_domain_spice_data(self, id, domain, remote_addr=False):
        try:
            ...
            viewer = (
                ...
                .get(domain["hypervisors_pools"][0])
                .run(db.conn)["viewer"]
            )

            if (
                viewer["defaultMode"] == "Secure"
                and domain["viewer"]["port_spice_ssl"] is not False
            ):
                tlsport = self.get_viewer_port(
                    domain["hyp_started"],
                    domain["viewer"]["port_spice_ssl"],
                    remote_addr,
                )
                port = self.get_viewer_port(
                    domain["hyp_started"], domain["viewer"]["port_spice"], remote_addr
                )
                ...
                return {
                    ...
                    "uri": "https://<domain>/wsviewer/spice"
                    + selfsigned
                    + "/?host="
                    + hostname
                    + "&port=5"
                    ...
                    + "&protocol=wss",
                    "viewers_options": domain["options"]["viewers"]["spice"]
                    if "spice" in domain["options"]["viewers"].keys()
                    else False,
                }
            if (
                viewer["defaultMode"] == "Insecure"
                and domain["viewer"]["port_spice"] is not False
            ):
                port = self.get_viewer_port(
                    domain["hyp_started"], domain["viewer"]["port_spice"], remote_addr
                )
                return {
                    ...
                    "host-subject": False,
                    "passwd": domain["viewer"]["passwd"],
                    "uri": "http://<domain>/wsviewer/spice/?host="
                    + hostname
                    + "&port=5"
                    + port
                    + "&passwd="
                    + domain["viewer"]["passwd"]
                    + "&protocol=ws",
                    "options": domain["options"]["viewers"]["spice"]
                    if "spice" in domain["options"]["viewers"].keys()
                    else False,
                }
            log.error(
                "No available Spice Viewer for domain " + id + " exception:" + str(e)
            )
            return False
    ...
    def get_domain_vnc_data(self, id, domain, remote_addr=False):
        try:
            ...
            port = self.get_viewer_port(
                domain["hyp_started"], domain["viewer"]["port_vnc"], remote_addr
            )
            selfsigned = "" if viewer["certificate"] is False else "_ss"
            ...
            if (
                ...
                return {
                    ...
                    "host-subject": viewer["host-subject"],
                    "passwd": domain["viewer"]["passwd"],
                    "uri": "https://<domain>/wsviewer/novnclite"
                    + selfsigned
                    + "/?host="
                    + hostname
                    + "&port=5"
                    ...
                    + "&password="
                    + domain["viewer"]["passwd"],
                    "options": domain["options"]["viewers"]["vnc"]
                    if "vnc" in domain["options"]["viewers"].keys()
                    else False,
                }
            if (
                viewer["defaultMode"] == "Insecure"
                ...
                return {
                    ...
                    "host-subject": viewer["host-subject"],
                    "passwd": domain["viewer"]["passwd"],
                    "uri": "http://<domain>/wsviewer/novnclite/?host="
                    + hostname
                    + "&port=5"
                    + port
                    + "&password="
            ...
            log.error(
                "No available VNC Viewer for domain " + id + " exception:" + str(e)
            )
            return False
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
    ...
    def get_spice_file(self, dict, id, remote_addr=False):
        ...
        try:
            op_fscr = (
                1
                if dict["options"] is not False and dict["options"]["fullscreen"]
                else 0
            )
        except:
            op_fscr = 0
        ...
        secure-channels=main;inputs;cursor;playback;record;display;usbredir;smartcard"""
                % (
                    "" if dict["host-subject"] is not False else ";",
                    dict["host-subject"],
                    "" if dict["ca"] is not False else ";",
                    dict["ca"],
                )
            )
    ...
    def get_viewer_hostname(self, viewer, remote_addr):
        if remote_addr is False:
            return viewer["hostname"]
        if (
            IPAddress(remote_addr).is_private()
            or not "hostname_external" in viewer.keys()
            or viewer["hostname_external"] is False
        ):
            return viewer["hostname"]
        else:
    ...
    def get_viewer_port(self, hypervisor, port, remote_addr):
        if remote_addr is False:
            return port  # viewer['hostname']

        if IPAddress(remote_addr).is_private():
            return port
        else:
            return int(port) + int(
                r.table("hypervisors").get(hypervisor).run(db.conn)["viewer_nat_offset"]
            )
